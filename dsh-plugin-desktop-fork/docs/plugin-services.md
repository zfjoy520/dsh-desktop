# DSH Desktop Fork plugin services

English | [中文](plugin-services.zh.md)

This document is the supported integration contract for plugin authors. It covers the public Host services `desktopProfiles` and `desktopPnpm`, plus the Client service `desktopWindow`, exported by DSH Desktop Fork 2.x in compatibility, extended, and advanced presentation modes. It does not grant third-party access to raw Electron APIs or launcher bootstrap state.
...
import type { DesktopWindowService } from 'dsh-plugin-desktop-fork/client'
...
Desktop marks the command bar with `data-dsh-desktop-fork-frame="titlebar"` and the upstream root with `data-dsh-desktop-fork-content-viewport`. The root is a separate fixed viewport below the command bar, so fixed descendants cannot escape into Desktop chrome. Full-viewport dialogs portalled directly to `document.body` receive the same content offset. Body-level plugin portals can read the `dsh-desktop-fork-titlebar-inset` URL contract; framed modes publish the exact 36-pixel reservation. Plugins must not compensate for a boundary they already consume.
...
import type {
  ...
  DesktopProfiles,
} from 'dsh-plugin-desktop-fork/profile-service'
import type {
  DesktopPnpm,
  DesktopPnpmHandle,
  DesktopPnpmOutcome,
} from 'dsh-plugin-desktop-fork/pnpm'
```

`dsh-plugin-desktop-fork/profiles` is the Desktop-owned tray consumer, not the profile service contract. Do not import it for service types.
...
| --- | --- | --- |
| `desktopProfiles` | Generation-scoped Host service. | Public and supported through `dsh-plugin-desktop-fork/profile-service`. |
| `desktopPnpm` | Generation-scoped Host service. | Public and supported through `dsh-plugin-desktop-fork/pnpm`. |
| `desktopWindow` | Generation-scoped Client service. | Public and supported through `dsh-plugin-desktop-fork/client`; immutable geometry only. |
...
A plugin that only makes sense inside DSH Desktop Fork can declare both services as required dependencies. Cordis keeps the plugin pending until both providers are available and unloads its effects if a required service disappears.

```ts
import type { Context } from '@deepseek-ai/cordis'
import type {} from 'dsh-plugin-desktop-fork/profile-service'
import type { DesktopPnpmHandle } from 'dsh-plugin-desktop-fork/pnpm'
...
import type { Context } from '@deepseek-ai/cordis'
import type {} from 'dsh-plugin-desktop-fork/profile-service'
import type {} from 'dsh-plugin-desktop-fork/pnpm'
...
Type-only imports are erased from JavaScript. A cross-environment package can keep `dsh-plugin-desktop-fork` as a development dependency for compilation, or as an optional peer if it publishes declarations that expose these types; it does not need a runtime import merely to probe the services.
...
yarn workspace dsh-plugin-desktop-fork verify:profile
```
...
The supported plugin-author surface is the `desktopProfiles`, `desktopPnpm`, and `desktopWindow` service contract described here and exported by `dsh-plugin-desktop-fork/profile-service`, `dsh-plugin-desktop-fork/pnpm`, and `dsh-plugin-desktop-fork/client`. Launcher bootstrap values, native adapters, generated shims, state-file formats, Loader row ordering, and Electron implementation details may change without becoming third-party APIs. Keep fallbacks explicit, lifecycle-scoped, and headless-safe.
