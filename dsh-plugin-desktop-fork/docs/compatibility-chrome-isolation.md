...
- A WebContentsView loads the upstream DSH page in `persist:dsh-desktop-fork-renderer`. It retains the existing file-path preload, authentication exchange, request capability, navigation policy, boot monitoring, reload, zoom, and developer-tools behavior.
