...
function verifyLifecycleEnvironment(stateRoot, installation, env) {
  ...
  writeFileSync(join(project, 'package.json'), JSON.stringify({
    name: 'dsh-desktop-fork-pnpm-lifecycle-smoke',
    version: '0.0.0',
    private: true,
...
function runPackagedPnpmShim() {
  const stateRoot = mkdtempSync(join(tmpdir(), 'dsh-desktop-fork-pnpm-smoke-'))
  const env = cleanEnvironment()
  let installation
  try {
...
function runFlatProfileDshEntry() {
  const root = mkdtempSync(join(tmpdir(), 'dsh-desktop-fork-flat-cli-smoke-'))
  const desktopPackage = join(root, 'node_modules', 'dsh-plugin-desktop-fork')
