...
const BIN_NAME = 'dsh-plugin-desktop-fork-profile-smoke'
const HOST_SERVICE_PLUGIN_NAME = 'dsh-desktop-fork-host-services-smoke-plugin'
...
try {
  ...
  const expectedUrl = `http://127.0.0.1:${String(ctx.webServer.port)}/?dsh-desktop-fork-mode=advanced&dsh-desktop-fork-platform=win32&dsh-desktop-fork-version=2.0.0&dsh-desktop-fork-material=off&dsh-desktop-fork-mica=1`
  ...
  if (desktopSettings?.mode !== 'advanced') {
    throw new Error('assembled Host settings are missing the advanced dsh-desktop-fork mode')
  }
