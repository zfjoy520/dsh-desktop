...
export async function verifyElectronExecutableFuses(
  ...
  try {
    ...
    throw new Error(`dsh-plugin-desktop-fork: failed to inspect Electron fuses at ${executable}`, {
      cause,
    })
  }
  ...
  if (invalid.length > 0) {
    throw new Error(
      `dsh-plugin-desktop-fork: final Electron executable at ${executable} has invalid required fuses: ${invalid.join(', ')}`,
    )
  }
}
...
function architectureNumber(name: string, description: string): Arch {
  try {
    ...
    throw new Error(
      `dsh-plugin-desktop-fork: unsupported Electron ${description} architecture ${JSON.stringify(name)}`,
      { cause },
    )
  }
}
...
function architectureName(arch: number, description: string): string {
  ...
  if (name === undefined) {
    throw new Error(
      `dsh-plugin-desktop-fork: unsupported Electron ${description} architecture ${String(arch)}`,
    )
  }
  return name
}
...
function requestedArchitectures(
  ...
  if (!(targets instanceof Map)) {
    throw new Error(
      `dsh-plugin-desktop-fork: cannot determine requested Electron architecture(s) for ${key}`,
    )
  }
  ...
  // --dir mac builds never populate the target map at all: MacPackager
  // skips mapper() for the dir target, so targetNames is empty even though
  // the configured target list still names 'dir'. Fall back to it (fork-local
  // fix; stable hits the same builder behavior).
  if (targetNames.size === 0) {
    const configured = result.configuration[key]?.target
    const entries = configured === undefined || configured === null
      ? []
      : Array.isArray(configured) ? configured : [configured]
    const names = entries.map(entry => typeof entry === 'string' ? entry : entry.target)
    if (names.some(name => name === DIR_TARGET || name.startsWith(`${DIR_TARGET}:`))) {
...
export function resolveFinalPackagedRuntimeContexts(
  ...
  const contexts = [...result.platformToTargets.entries()].flatMap(([platform, targets]) => {
    ...
    if (key !== 'linux' && key !== 'mac' && key !== 'win') {
      throw new Error(
        `dsh-plugin-desktop-fork: unsupported Electron build platform ${JSON.stringify(platform.buildConfigurationKey)}`,
      )
    }
  ...
  if (missing.length > 0) {
    throw new Error(
      `dsh-plugin-desktop-fork: cannot locate final Electron executable(s) for ${missing.sort().join(', ')}`,
    )
  }
