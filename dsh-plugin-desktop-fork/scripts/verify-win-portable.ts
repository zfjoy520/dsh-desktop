...
export function verifyWindowsPortable(
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'DSH Desktop Fork.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing DSH Desktop Fork.exe: ${portablePath}`)
  }
  ...
  assertPortableExecutableBuffer(
    ...
    `${portablePath}:DSH Desktop Fork.exe`,
  )
  return portablePath
}
