...
$taskExistingInstalls = @(Get-ItemProperty $taskUninstallRoots -ErrorAction SilentlyContinue | Where-Object {
  $_.DisplayName -match '^DSH Desktop Fork'
})

if ($taskExistingProcesses.Count -gt 0 -or $taskExistingInstalls.Count -gt 0) {
  throw 'Refusing to run while an existing DSH Desktop Fork process or installation is present.'
}
...
$taskUserData = Join-Path $env:APPDATA 'DSH Desktop Fork'
$taskDshHome = Join-Path $taskRoot 'dsh-home'
$taskActiveRunMarker = Join-Path $taskUserData 'crash-evidence\active-run.json'
$taskAppPath = Join-Path $taskInstallRoot 'DSH Desktop Fork.exe'
$taskUninstallerPath = Join-Path $taskInstallRoot 'Uninstall DSH Desktop Fork.exe'
if (Test-Path -LiteralPath $taskActiveRunMarker) {
  throw 'Refusing to overwrite an existing DSH Desktop Fork active run marker.'
}
$taskResult = [ordered]@{
...
function Wait-TaskProcess([bool]$taskShouldExist, [int]$taskTimeoutSeconds = 30) {
  ...
  throw "Timed out waiting for DSH Desktop Fork process state: shouldExist=$taskShouldExist"
}
...
try {
  ...
  $taskRemainingShortcuts = @($taskShortcutRoots | ForEach-Object {
    Get-ChildItem -LiteralPath $_ -Filter '*DSH Desktop Fork*' -Recurse -ErrorAction SilentlyContinue
  })
