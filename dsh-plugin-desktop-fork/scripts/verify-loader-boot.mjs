...
const BIN_NAME = 'dsh-plugin-desktop-fork-loader-smoke'
const THIRD_PARTY_NAME = 'dsh-desktop-fork-loader-smoke-plugin'
const THIRD_PARTY_DEPENDENCY_NAME = 'dsh-desktop-fork-loader-smoke-dependency'
...
const home = mkdtempSync(join(tmpdir(), 'dsh-desktop-fork-loader-'))
let ctx
let mounted
let mountedSpec
...
try {
  ...
  if (profileRequire.resolve('dsh-plugin-desktop-fork/package.json') !== desktopManifest) {
  ...
  if (profileDirectoryRequire.resolve('dsh-plugin-desktop-fork/package.json') !== desktopManifest) {
  ...
  ctx = await boot(
    ...
    prepared.rootConfig,
    [{ insert: [
      { id: 'desktop-shell', name: 'dsh-plugin-desktop-fork' },
  ...
  if (desktopEntry?.options.name !== 'dsh-plugin-desktop-fork') {
  ...
  const expectedUrl = `http://127.0.0.1:43120/?dsh-desktop-fork-mode=compatibility&dsh-desktop-fork-platform=darwin&dsh-desktop-fork-version=${PRODUCT_VERSION}&dsh-desktop-fork-material=transparent&dsh-desktop-fork-titlebar-inset=36`
