...
export function resolveRuntimePackageRoot(
  ...
  while (true) {
    ...
    if (directory === root) {
      throw new Error(`dsh-plugin-desktop-fork: cannot locate package root for ${packageName}`, {
        cause: manifestCause,
      })
    }
...
export function resolvePackagedExecutablePath(context: PackagedRuntimeContext): string {
  ...
  throw new Error(
    `dsh-plugin-desktop-fork: unsupported Electron afterPack platform ${JSON.stringify(context.electronPlatformName)}`,
  )
}
  ...
  try {
    for (const check of checks) {
      ...
      if (result.error !== undefined) {
        throw new Error(`dsh-plugin-desktop-fork: packaged ${check.label} smoke could not start`, {
          cause: result.error,
        })
      }
      if (result.status !== 0 || !check.accepts(result.stdout)) {
        throw new Error(
          `dsh-plugin-desktop-fork: packaged ${check.label} smoke failed with status ${String(result.status)}; `
    ...
    if (usesAsarLayout(context)
      ...
      throw new Error(
        `dsh-plugin-desktop-fork: packaged Desktop CLI recreated the obsolete shared Profile fallback at ${obsoleteFallback}`,
      )
    }
  } finally {
...
function normalizeArchiveEntry(entry: string): string {
  ...
  if (segments.length === 0 || segments.includes('..')) {
    throw new Error(`dsh-plugin-desktop-fork: invalid packaged runtime path ${JSON.stringify(entry)}`)
  }
...
export function indexPackagedAsarHeader(header: unknown): PackagedAsarIndex {
  ...
  const visit = (
    ...
    if (!record(value)) {
      throw new Error(`dsh-plugin-desktop-fork: malformed ASAR header entry at ${path ?? '(root)'}`)
    }
    if ('unpacked' in value && typeof value.unpacked !== 'boolean') {
      throw new Error(`dsh-plugin-desktop-fork: malformed ASAR unpacked flag at ${path ?? '(root)'}`)
    }
    const unpacked = parentUnpacked || value.unpacked === true
    if ('files' in value) {
      if (!record(value.files) || 'link' in value || 'size' in value) {
        throw new Error(`dsh-plugin-desktop-fork: malformed ASAR directory entry at ${path ?? '(root)'}`)
      }
    ...
    if (path === undefined || (!('size' in value) && !('link' in value))) {
      throw new Error(`dsh-plugin-desktop-fork: malformed ASAR leaf entry at ${path ?? '(root)'}`)
    }
    if (files.has(path)) {
      throw new Error(`dsh-plugin-desktop-fork: duplicate normalized ASAR header entry ${path}`)
    }
    files.add(path)
...
export function verifyPackagedAsar(
  ...
  try {
    ...
    throw new Error(
      `dsh-plugin-desktop-fork: failed to inspect packaged runtime at ${archivePath}`,
      { cause },
    )
  }
  ...
  if (missing.length > 0) {
    throw new Error(
      `dsh-plugin-desktop-fork: packaged runtime at ${archivePath} is missing required ASAR entries: ${missing.join(', ')}`,
    )
  }
  return index
}
...
export function summarizeUnpackedRuntime(
  ...
  for (const file of files) {
    if (!Number.isSafeInteger(file.bytes) || file.bytes < 0) {
      throw new Error(
        `dsh-plugin-desktop-fork: unpacked runtime entry ${JSON.stringify(file.path)} has invalid byte size ${String(file.bytes)}`,
      )
    }
    bytes += file.bytes
...
export function verifySelectiveUnpackedRuntime(
  ...
  if (duplicates.size > 0) {
    throw new Error(
      `dsh-plugin-desktop-fork: unpacked runtime at ${unpackedRoot} contains duplicate normalized physical entries: ${[...duplicates].sort().join(', ')}`,
    )
  }
  ...
  if (mismatches.length > 0) {
    throw new Error(
      `dsh-plugin-desktop-fork: unpacked runtime at ${unpackedRoot} does not exactly match ASAR unpacked flags: ${mismatches.join('; ')}; inventory: ${inventory}`,
    )
  }
  ...
  if (desktopCode.length > 0) {
    throw new Error(
      `dsh-plugin-desktop-fork: unpacked runtime at ${unpackedRoot} contains desktop code/data that must stay in app.asar: ${desktopCode.join(', ')}; inventory: ${inventory}`,
    )
  }
  ...
  if (forbidden.length > 0) {
    throw new Error(
      `dsh-plugin-desktop-fork: unpacked runtime at ${unpackedRoot} mirrors ordinary archived modules: ${forbidden.join(', ')}; inventory: ${inventory}`,
    )
  }
  ...
  if (unexpectedDesktopEntries.length > 0) {
    throw new Error(
      `dsh-plugin-desktop-fork: unpacked runtime at ${unpackedRoot} contains non-allowlisted desktop entries: ${unexpectedDesktopEntries.join(', ')}; inventory: ${inventory}`,
    )
  }
  ...
  if (unexpectedPackageRoots.length > 0) {
    throw new Error(
      `dsh-plugin-desktop-fork: unpacked runtime at ${unpackedRoot} contains non-allowlisted package roots: ${unexpectedPackageRoots.join(', ')}; inventory: ${inventory}`,
    )
  }
  ...
  if (pnpm !== undefined
    ...
    throw new Error(
      `dsh-plugin-desktop-fork: unpacked runtime at ${unpackedRoot} exceeds pnpm smart-unpack budget `
  ...
  if (summary.files > MAX_UNPACKED_RUNTIME_FILES) {
    throw new Error(
      `dsh-plugin-desktop-fork: unpacked runtime at ${unpackedRoot} exceeds selective ASAR file budget ${String(MAX_UNPACKED_RUNTIME_FILES)}; inventory: ${inventory}`,
    )
  }
  if (summary.bytes > MAX_UNPACKED_RUNTIME_BYTES) {
    throw new Error(
      `dsh-plugin-desktop-fork: unpacked runtime at ${unpackedRoot} exceeds selective ASAR byte budget ${String(MAX_UNPACKED_RUNTIME_BYTES)}; inventory: ${inventory}`,
    )
  }
  return summary
}
...
export function verifyPackagedRuntime(
  ...
  try {
    ...
    throw new Error(
      `dsh-plugin-desktop-fork: failed to inventory desktop build output at ${libRoot}`,
      { cause },
    )
  }
  ...
  if (context.electronPlatformName === 'win32' && context.arch !== undefined && context.arch !== 1) {
    throw new Error(
      `dsh-plugin-desktop-fork: unsupported Windows package architecture ${String(context.arch)}; only x64 is configured`,
    )
  }
  ...
  if (missing.length > 0) {
    throw new Error(
      `dsh-plugin-desktop-fork: packaged runtime at ${runtimeRoot} is missing required physical entries: ${missing.join(', ')}`,
    )
  }
  if (context.electronPlatformName === 'darwin' && context.arch === 4) {
    ...
    if (forbidden.length > 0) {
      throw new Error(
        `dsh-plugin-desktop-fork: universal macOS runtime at ${runtimeRoot} contains host-architecture build output: ${forbidden.join(', ')}`,
      )
    }
  }
...
export function reportUnpackedRuntime(summary: UnpackedRuntimeSummary): void {
  process.stdout.write(`dsh-plugin-desktop-fork: packaged runtime inventory: ${formatUnpackedRuntimeSummary(summary)}\n`)
}

/**
 * Fork-local: drop Electron's stock `default_app.asar` placeholder from the
 * staged bundle. Official artifacts ship without it, but `--dir` builds keep
 * the stock file and the packaged filesystem smoke trips over Electron's ASAR
 * stat shim for it (the vendored fs backend expects BigInt stat fields). The
 * placeholder is never executed because `Resources/app` exists.
 */
export function pruneStockDefaultApp(context: PackagedRuntimeContext): void {
  if (context.electronPlatformName !== 'darwin') return
  rmSync(join(dirname(resolvePackagedApplicationRoot(context)), 'default_app.asar'), { force: true })
}
