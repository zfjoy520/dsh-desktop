...
const MAIN_WINDOW_STATE_PATH = '/tmp/dsh-desktop-fork-user-data/main-window-state.json'
...
const electron = vi.hoisted(() => {
  ...
  class WebContentsView {
    ...
    constructor(readonly options: unknown) {
      this.webContents = (options as { webPreferences: { partition: string } }).webPreferences.partition === 'dsh-desktop-fork-compatibility-chrome'
  ...
  class BrowserWindow {
    ...
    constructor(options: unknown) {
      this.webContents = (options as { webPreferences?: { partition?: string } }).webPreferences?.partition === 'dsh-desktop-fork-compatibility-host'
        ? chromeWebContents : webContents
  ...
  return {
    app: {
      ...
      getPath: vi.fn((name: string) => {
        if (name === 'crashDumps') return '/tmp/dsh-desktop-fork-user-data/Crashpad'
        if (name === 'downloads') return '/tmp/Downloads'
        return '/tmp/dsh-desktop-fork-user-data'
      }),
    ...
    Menu: {
      buildFromTemplate: vi.fn((template: unknown[]) => {
        ...
        if (labels.includes('Unread') || labels.includes('未读会话')
          || (labels.length === 1 && (labels[0] === 'Open DSH Desktop Fork' || labels[0] === '打开 DSH Desktop Fork'))) {
          dockMenuTemplates.push(template)
          return { template }
        }
        const first = template[0] as { label?: unknown, submenu?: unknown } | undefined
        if (first?.label === 'DSH Desktop Fork' && Array.isArray(first.submenu)) {
...
const spec: DesktopShellSpec = {
  ...
  rendererAccessHeader: {
    name: 'x-dsh-desktop-fork-renderer',
...
describe('Electron desktop runtime', () => {
  ...
  it('uses the independent macOS compatibility frame, Dock icon, and template tray image', async () => {
    ...
    expect(options).toEqual(expect.objectContaining({
      ...
      webPreferences: {
        ...
        sandbox: true,
        webSecurity: true,
        partition: 'dsh-desktop-fork-compatibility-host',
      },
    }))
    ...
    expect(electron.contentViews[1]?.options).toEqual({ webPreferences: {
      ...
      sandbox: true,
      webSecurity: true,
      partition: 'persist:dsh-desktop-fork-renderer',
    } })
  ...
  it('attaches the renderer capability to same-origin HTTP and WebSocket requests only', async () => {
    ...
    listener({
      ...
      requestHeaders: {
        Accept: '*/*',
        // Spoof the header name the fork runtime honors (stable-named
        // headers are inert here); it must still be stripped and re-attached.
        [spec.rendererAccessHeader.name]: 'spoofed',
      },
    }, assetCallback)
  ...
  it('restores, debounces, and flushes main-window bounds across shell generations', async () => {
    ...
    const store = new FileMainWindowStateStore('/tmp/dsh-desktop-fork-user-data')
  ...
  it('blocks unsupported workspace volumes without returning a risky path', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith('dsh-plugin-desktop-fork: workspace volume decision=blocked path=E:\\repo')
  })
  ...
  it('requires explicit confirmation for a removable NTFS workspace', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith('dsh-plugin-desktop-fork: workspace volume decision=cancelled path=E:\\repo')

    electron.dialog.showMessageBox.mockResolvedValue({ response: 0, checkboxChecked: false })
    await expect(runtime.validateDirectory('E:\\repo')).resolves.toBe(true)
    expect(logger.error).toHaveBeenCalledWith('dsh-plugin-desktop-fork: workspace volume decision=confirmed path=E:\\repo')
  })
  ...
  it('offers restart first when the supervised Host is gone, and names the exit code', async () => {
    ...
    expect(electron.dialog.showMessageBox).toHaveBeenCalledWith(expect.objectContaining({
      type: 'error',
      buttons: ['Restart DSH Desktop Fork', 'Open DSH Terminal', 'Dismiss'],
      defaultId: 0,
      cancelId: 2,
  ...
  it('logs renderer crashes with the Windows exception code', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'dsh-plugin-desktop-fork: renderer process gone (reason: crashed, exitCode: -1073741819 / 0xc0000005)',
    )
    await release()
  })
  ...
  describe('runtime renderer recovery', () => {
    ...
    it.each(['oom', 'crashed', 'abnormal-exit'] as const)('silently reloads after %s without restarting the Host or revealing a hidden window', async (reason) => {
      ...
      expect(runtime.rendererBootFailureReason).toBeUndefined()
      expect(logger.error).toHaveBeenCalledWith('dsh-plugin-desktop-fork: automatic renderer recovery healthy')
      await release()
    })
    ...
    it('recovers an isolated chrome crash and waits for both documents', async () => {
      ...
      runtime.reportRendererBoot({ status: 'healthy' })
      expect(logger.error).not.toHaveBeenCalledWith('dsh-plugin-desktop-fork: automatic renderer recovery healthy')
      healthy()
      expect(logger.error).toHaveBeenCalledWith('dsh-plugin-desktop-fork: automatic renderer recovery healthy')
      await vi.advanceTimersByTimeAsync(90_000)
      expect(electron.dialog.showMessageBox).not.toHaveBeenCalled()
    ...
    it('replaces a persistently unresponsive renderer and reloads once its exit lands', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        'dsh-plugin-desktop-fork: terminating unresponsive renderer; the crash dump it produces is deliberate',
      )
    ...
    it('logs a fallback dialog failure and permits another attempt from the tray', async () => {
      ...
      await exhaust()
      expect(logger.error).toHaveBeenCalledWith('dsh-plugin-desktop-fork: renderer recovery failed: dialog unavailable')
      runtime.show()
      await Promise.resolve()
      await vi.advanceTimersByTimeAsync(0)
      expect(electron.webContents.reloadIgnoringCache).toHaveBeenCalledTimes(4)
  ...
  it('keeps external window links deny-by-default with a narrow protocol allowlist', async () => {
    ...
    await Promise.resolve()
    expect(logger.error).toHaveBeenCalledWith(
      'dsh-plugin-desktop-fork: failed to open external link: external handler unavailable',
    )
  ...
  it('rejects an unsupported Electron platform before creating a runtime', async () => {
    ...
    expect(() => new ElectronDesktopRuntime(async () => {})).toThrow(
      'dsh-plugin-desktop-fork: unsupported Electron platform aix',
    )
    expect(electron.browserWindowOptions).toHaveLength(0)
  })
  ...
  it('rebuilds ordered effect-scoped tray contributions without replacing native commands', async () => {
    ...
    expect(labels).toEqual([
      'Open DSH Desktop Fork', 'Reload Interface', undefined,
  ...
  it('renders contributed radio submenus in their own profile section', async () => {
    ...
    }>).find(item => item.label === 'DSH Desktop Fork')
  ...
  it('opens the active profile through the packaged terminal adapter', async () => {
    ...
    try {
      ...
      expect(terminal.open).toHaveBeenCalledWith(expect.objectContaining({
        ...
        homeDir: expect.stringContaining('dsh-desktop-fork-user-data'),
  ...
  it('coalesces concurrent diagnostic exports and reveals the completed archive', async () => {
    ...
    expect(diagnostics.export).toHaveBeenCalledWith(
      expect.stringContaining('dsh-desktop-fork-user-data'),
  ...
  it('shows native recovery when the renderer Loader reports a failed plugin', async () => {
    ...
    expect(electron.dialog.showMessageBox).toHaveBeenCalledWith(expect.objectContaining({
      ...
      buttons: ['Open DSH Terminal', 'Restart DSH Desktop Fork', 'Dismiss'],
    }))
  ...
  it('logs the renderer boot failure details for diagnostics', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'dsh-plugin-desktop-fork: renderer boot failed (plugins: dsh-vision-router): '
  ...
  it('requires Desktop dialog confirmation before an ordinary restart', async () => {
    ...
    expect(restart).not.toHaveBeenCalled()
    expect(electron.dialog.showMessageBox).toHaveBeenCalledWith(expect.objectContaining({
      type: 'question', title: 'Restart DSH Desktop Fork', buttons: ['Restart', 'Cancel'], defaultId: 1, cancelId: 1,
    }))
  ...
  it('uses Electron networking and confirmation-gated macOS update handoff', async () => {
    ...
    expect(runtime.updates).toMatchObject({
      ...
      statePath: join('/tmp/dsh-desktop-fork-user-data', 'updates', 'state.json'),
    })
    electron.app.isPackaged = true
    ...
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(
      activeWindow,
      expect.objectContaining({
        title: 'DSH Desktop Fork Is Up to Date',
    ...
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-fork-user-data', {
      platform: 'darwin',
      version: '2.1.0',
    ...
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(
      activeWindow,
      expect.objectContaining({
        title: 'DSH Desktop Fork Update Downloaded',
        buttons: ['OK'],
      }),
    )
  ...
  it('continues the update handoff when cleanup tracking cannot be persisted', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'dsh-plugin-desktop-fork: failed to remember update installer for cleanup: read-only user data',
    )
    expect(childProcess.spawn).not.toHaveBeenCalled()
  })
  ...
  it.each([
    ...
    expect(updater.resolve).toHaveBeenCalledWith('/tmp/dsh-desktop-fork-user-data', artifact, remove)
  })
