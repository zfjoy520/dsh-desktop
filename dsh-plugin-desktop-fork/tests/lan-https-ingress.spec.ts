...
beforeAll(async () => {
  const ca = await generate([{ name: 'commonName', value: 'DSH Desktop Fork test root' }], {
    algorithm: 'sha256',
    keyType: 'ec',
    extensions: [
...
describe('LAN HTTPS ingress', () => {
  ...
  it('streams a large request body and preserves browser authority while removing spoofable identity headers', async () => {
    ...
    const result = await httpsCall(ingress, '/upload', {
      ...
      headers: {
        ...
        'x-dsh-desktop-fork-renderer': 'forged-capability',
    ...
    expect(receivedHeaders?.['x-dsh-desktop-fork-renderer']).toBeUndefined()
  ...
  it('passes upgrade head and both socket directions without forwarding a renderer capability', async () => {
    ...
    expect(upgradeHeaders?.['x-dsh-desktop-fork-renderer']).toBeUndefined()
