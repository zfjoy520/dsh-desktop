...
describe('ElectronStderrLogger', () => {
  it('logs Electron child process crashes with the Windows exception code', () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'dsh-plugin-desktop-fork: child process gone (type: Utility, name: Network Service, service: network.mojom.NetworkService, reason: crashed, exitCode: -1073741819 / 0xc0000005)',
    )
    remove()
  ...
  it('accepts fail-loud stderr chunks without adding a second newline', () => {
    ...
    logger.write('dsh-plugin-desktop-fork: fatal load failure: Bearer abc.def.secret\n')

    const day = todaySuffix()
    const text = readFileSync(join(dir, `dsh-${day}.log`), 'utf8')
    expect(text).toContain('fatal load failure: Bearer ****\n')
    expect(text).not.toContain('abc.def.secret')
    expect(stderrSpy).toHaveBeenCalledWith('dsh-plugin-desktop-fork: fatal load failure: Bearer ****\n')
    stderrSpy.mockRestore()
  })
