...
function options(overrides: Partial<MacReleaseVerificationOptions> = {}) {
  ...
  const value: MacReleaseVerificationOptions = {
    ...
    listDmgs: () => ['/release/dist/DSH Desktop Fork-2.0.0-universal.dmg'],
    makeMountPoint: () => '/private/tmp/dsh-desktop-fork-dmg-test',
...
describe('macOS release artifact verification', () => {
  it('mounts one DMG and verifies signature, Gatekeeper, and the stapled ticket', () => {
    ...
    const appPath = join('/private/tmp/dsh-desktop-fork-dmg-test', 'DSH Desktop Fork.app')

    expect(verifyMacRelease(harness.value)).toEqual({
      appPath,
      dmgPath: '/release/dist/DSH Desktop Fork-2.0.0-universal.dmg',
    })
    ...
    expect(harness.calls).toEqual([
      {
        command: 'hdiutil',
        args: [
          'attach', '/release/dist/DSH Desktop Fork-2.0.0-universal.dmg',
          '-mountpoint', '/private/tmp/dsh-desktop-fork-dmg-test', '-nobrowse', '-readonly',
        ],
      },
      {
        command: 'lipo',
        args: [join(appPath, 'Contents', 'MacOS', 'DSH Desktop Fork'), '-verify_arch', 'x86_64'],
      },
      {
        command: 'lipo',
        args: [join(appPath, 'Contents', 'MacOS', 'DSH Desktop Fork'), '-verify_arch', 'arm64'],
      },
      ...
      {
        command: 'hdiutil',
        args: ['detach', '/private/tmp/dsh-desktop-fork-dmg-test'],
      },
    ])
    expect(harness.removeMountPoint).toHaveBeenCalledWith('/private/tmp/dsh-desktop-fork-dmg-test')
  })
