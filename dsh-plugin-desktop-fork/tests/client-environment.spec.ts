...
describe('desktop client environment', () => {
  it.each(['darwin', 'win32', 'linux'])('keeps compatibility chrome out of the %s client slot tree', platform => {
    const marker = platform === 'win32' ? '&dsh-desktop-fork-mica=0' : ''
    vi.stubGlobal('window', { location: {
      search: `?dsh-desktop-fork-platform=${platform}&dsh-desktop-fork-mode=compatibility&dsh-desktop-fork-version=2.0.3&dsh-desktop-fork-material=off${marker}`,
    } })
    const effect = vi.fn()
    const inject = vi.fn()
    const ctx = {
  ...
  it('accepts the Electron-owned kebab query markers', () => {
    expect(parseDesktopClientEnvironment('?dsh-desktop-fork-mode=advanced&dsh-desktop-fork-platform=darwin&dsh-desktop-fork-version=2.0.3&dsh-desktop-fork-material=transparent'))
      .toEqual({ version: '2.0.3', mode: 'advanced', platform: 'darwin', material: 'transparent', micaSupported: false })
    expect(parseDesktopClientEnvironment('?dsh-desktop-fork-platform=win32&dsh-desktop-fork-mode=compatibility&dsh-desktop-fork-version=2.0.3&dsh-desktop-fork-material=off&dsh-desktop-fork-mica=0'))
      .toEqual({ version: '2.0.3', mode: 'compatibility', platform: 'win32', material: 'off', micaSupported: false })
    expect(parseDesktopClientEnvironment('?dsh-desktop-fork-mode=extended&dsh-desktop-fork-platform=win32&dsh-desktop-fork-version=2.0.3&dsh-desktop-fork-material=mica&dsh-desktop-fork-mica=1'))
      .toEqual({ version: '2.0.3', mode: 'extended', platform: 'win32', material: 'mica', micaSupported: true })
    expect(parseDesktopClientEnvironment('?dsh-desktop-fork-mode=extended&dsh-desktop-fork-platform=win32&dsh-desktop-fork-version=2.0.3&dsh-desktop-fork-material=acrylic&dsh-desktop-fork-mica=0'))
      .toEqual({ version: '2.0.3', mode: 'extended', platform: 'win32', material: 'off', micaSupported: false })
    expect(parseDesktopClientEnvironment('?dsh-desktop-fork-mode=compatibility&dsh-desktop-fork-platform=linux&dsh-desktop-fork-version=2.0.3&dsh-desktop-fork-material=off'))
      .toEqual({ version: '2.0.3', mode: 'compatibility', platform: 'linux', material: 'off', micaSupported: false })
  })

  it.each([
    ['?dsh-desktop-fork-mode=glass&dsh-desktop-fork-platform=darwin', 'dsh-desktop-fork-mode'],
    ['?dsh-desktop-fork-mode=advanced', 'dsh-desktop-fork-platform'],
    ['?dsh-desktop-fork-platform=darwin', 'dsh-desktop-fork-mode'],
    ['?dsh-desktop-fork-mode=advanced&dsh-desktop-fork-platform=android', 'dsh-desktop-fork-platform'],
    ['?dsh-desktop-fork-mode=advanced&dsh-desktop-fork-platform=darwin', 'dsh-desktop-fork-material'],
    ['?dsh-desktop-fork-mode=advanced&dsh-desktop-fork-platform=darwin&dsh-desktop-fork-material=off', 'dsh-desktop-fork-version'],
    ['?dsh-desktop-fork-mode=advanced&dsh-desktop-fork-platform=win32&dsh-desktop-fork-version=2.0.3&dsh-desktop-fork-material=mica&dsh-desktop-fork-mica=0', 'incompatible'],
    ['?dsh-desktop-fork-mode=compatibility&dsh-desktop-fork-platform=linux&dsh-desktop-fork-version=2.0.3&dsh-desktop-fork-material=mica', 'incompatible'],
    ['?dsh-desktop-fork-mode=compatibility&dsh-desktop-fork-platform=linux&dsh-desktop-fork-version=2.0.3&dsh-desktop-fork-material=transparent', 'incompatible'],
...
describe('independent Desktop frame', () => {
  it('fills the native content viewport for both framed modes and limits the inverted-L surface to extended mode', () => {
    ...
    try {
      ...
      expect(css).toContain(`--dsh-desktop-fork-frame-height: 0px`)
      ...
      expect(css).toMatch(/\[role="presentation"\]:has\(> \[aria-modal="true"\]\),[\s\S]*> \[aria-modal="true"\] \{[\s\S]*top: var\(--dsh-desktop-fork-frame-height\) !important;/)
      expect(css).not.toContain('#root > :has(> [data-shell-overlay])')
      expect(css).toMatch(/body\[data-dsh-desktop-fork-mode="extended"\] \.dshDesktopSidebarSurface \{[^}]*--dsw-specific-sidebar-fill: transparent;[^}]*border-right-color: transparent;[^}]*background: transparent !important;/)
      expect(css).toMatch(/body\[data-dsh-desktop-fork-mode="extended"\] \.dshDesktopFrame \{[^}]*background: var\(--dsh-desktop-fork-frame-fill\);/)
      expect(css).toMatch(/body\[data-dsh-desktop-fork-mode="extended"\] \.dshDesktopConversationSurface \{[^}]*border-top: 1px solid var\(--dsw-alias-border-l1\);[^}]*border-left: 1px solid var\(--dsw-alias-border-l1\);[^}]*border-top-left-radius: 10px;/)
      expect(css).toContain('body:is([data-dsh-desktop-fork-mode="compatibility"], [data-dsh-desktop-fork-mode="extended"]) #root')
...
describe('sidebar footer stacking', () => {
  it.each(['compatibility', 'extended', 'advanced'])('owns the footer seat in %s mode', mode => {
    vi.stubGlobal('window', { location: {
      search: `?dsh-desktop-fork-platform=darwin&dsh-desktop-fork-mode=${mode}&dsh-desktop-fork-version=2.0.3&dsh-desktop-fork-material=off`,
    } })
    const effect = vi.fn()
    const ctx = {
      effect,
    ...
    try {
      ...
      expect(effect.mock.calls.map(([, label]) => label))
        .toContain('dsh-plugin-desktop-fork: sidebar footer stacking styles')
    }
    finally {
      vi.unstubAllGlobals()
    }
  ...
  it('stacks launchers in one bounded seat without shaving their rounded corners', () => {
    ...
    try {
      ...
      expect(css).not.toContain('data-dsh-desktop-fork-mode')
      expect(appendChild).toHaveBeenCalledWith(style)
      dispose()
      expect(remove).toHaveBeenCalledOnce()
    }
