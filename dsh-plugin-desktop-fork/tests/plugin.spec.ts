...
function createHarness(
  ...
  const runtime: DesktopRuntime = {
    ...
    updates: {
      ...
      statePath: '/tmp/dsh-desktop-fork-update-state.json',
...
describe('desktop Host plugin', () => {
  it('defaults to compatibility mode and validates both schemas', () => {
    ...
    expect(String(DESKTOP_SETTINGS_NAMESPACE)).toBe('dsh-desktop-fork')
  })
  ...
  it('builds the loopback root with validated renderer mode and platform markers', () => {
    ...
    expect(Object.fromEntries(url.searchParams)).toEqual({
      'dsh-desktop-fork-mode': 'advanced',
      'dsh-desktop-fork-platform': 'darwin',
      'dsh-desktop-fork-version': '2.0.3',
      'dsh-desktop-fork-material': 'off',
    })
    expect(Object.fromEntries(new URL(desktopRendererUrl(
      ...
      'dsh-desktop-fork-mode': 'extended',
      'dsh-desktop-fork-platform': 'win32',
      'dsh-desktop-fork-version': '2.0.3',
      'dsh-desktop-fork-material': 'mica',
      'dsh-desktop-fork-titlebar-inset': '36',
      'dsh-desktop-fork-mica': '1',
    })
    expect(Object.fromEntries(new URL(desktopRendererUrl(
      43120,
      'compatibility',
      'linux',
      '2.0.3',
    )).searchParams)).not.toHaveProperty('dsh-desktop-fork-titlebar-inset')
  })
  ...
  it('registers settings and the active Web port without re-entering Loader settlement', async () => {
    ...
    expect(harness.shell()).toEqual(expect.objectContaining({
      mode: 'compatibility',
      url: 'http://127.0.0.1:43120/?dsh-desktop-fork-mode=compatibility&dsh-desktop-fork-platform=darwin&dsh-desktop-fork-version=2.0.0&dsh-desktop-fork-material=transparent&dsh-desktop-fork-titlebar-inset=36',
