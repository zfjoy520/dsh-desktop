...
describe('Desktop installer quit request', () => {
  it('accepts only the dedicated flag on Windows', () => {
    ...
    expect(isDesktopInstallerQuitRequest(
      ['DSH Desktop Fork.exe', DESKTOP_INSTALLER_QUIT_FLAG],
      'win32',
    )).toBe(true)
    expect(isDesktopInstallerQuitRequest(['DSH Desktop Fork.exe', '--quit'], 'win32')).toBe(false)
    expect(isDesktopInstallerQuitRequest(
      ['DSH Desktop Fork', DESKTOP_INSTALLER_QUIT_FLAG],
      'darwin',
    )).toBe(false)
  })

  it('distinguishes background Node re-entry from an explicit application launch', () => {
    expect(isDesktopBackgroundNodeRequest(['DSH Desktop Fork.exe'])).toBe(false)
    expect(isDesktopBackgroundNodeRequest(['DSH Desktop Fork.exe', '--profile', 'desktop'])).toBe(false)
    expect(isDesktopBackgroundNodeRequest(['DSH Desktop Fork.exe', 'C:\\app\\pnpm\\bin\\pnpm.mjs', 'install'])).toBe(true)
    expect(isDesktopBackgroundNodeRequest(['DSH Desktop Fork.exe', '--require', 'C:\\runtime\\clear-env.cjs'])).toBe(true)
    expect(isDesktopBackgroundNodeRequest(['DSH Desktop Fork.exe', '--import=file:///runtime/clear-env.mjs'])).toBe(true)
    expect(isDesktopBackgroundNodeRequest(['DSH Desktop Fork.exe', '--expose-internals', 'desktop-cli.js'])).toBe(true)
  })
