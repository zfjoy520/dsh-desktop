...
describe('DesktopDialogWindow', () => {
  ...
  it('accepts only bounded local response navigation', () => {
    expect(parseDesktopDialogResponse('dsh-desktop-fork-dialog://response?id=1', 2)).toBe(1)
    expect(parseDesktopDialogResponse('dsh-desktop-fork-dialog://response?id=2', 2)).toBeUndefined()
    expect(parseDesktopDialogResponse('dsh-desktop-fork-dialog://response?id=-1', 2)).toBeUndefined()
    expect(parseDesktopDialogResponse('dsh-desktop-fork-dialog://response?id=1&command=bad', 2)).toBeUndefined()
  ...
  it('creates a frameless parented modal shadcn window and returns its explicit response', async () => {
    ...
    navigate?.(event, 'dsh-desktop-fork-dialog://response?id=0')
  ...
  it('uses a larger scrollable presentation for detailed recovery failures', async () => {
    ...
    window?.webListeners.get('will-navigate')?.(
      ...
      'dsh-desktop-fork-dialog://response?id=0',
    )
    await expect(result).resolves.toEqual({ response: 0 })
  })
  ...
  it('sizes the macOS Profile compatibility notice to its content without extra height', async () => {
    ...
    const result = new DesktopDialogWindow({
      ...
      message: 'Current Profile “work” was used by DSH Desktop Fork Beta.',
    ...
    window?.webListeners.get('will-navigate')?.(
      { preventDefault: vi.fn() },
      'dsh-desktop-fork-dialog://response?id=2',
    )
