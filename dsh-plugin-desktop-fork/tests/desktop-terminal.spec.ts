...
function temporaryDirectory(): string {
  const dir = mkdtempSync(join(tmpdir(), 'dsh-desktop-fork-terminal-'))
...
function macOptions(stateDir: string, spawn: DesktopTerminalSpawn): DesktopTerminalOptions {
  return {
    platform: 'darwin',
    appExecutable: "/Applications/DSH O'Brien.app/Contents/MacOS/DSH Desktop Fork",
...
function windowsOptions(stateDir: string, spawn: DesktopTerminalSpawn): DesktopTerminalOptions {
  return {
    platform: 'win32',
    appExecutable: 'C:\\Program Files\\DSH 100% Desktop\\DSH Desktop Fork.exe',
    dshBootstrapPath: 'C:\\Program Files\\DSH Desktop Fork\\resources\\app.asar\\lib\\dsh-terminal-bootstrap.js',
    pnpmBinPath: 'C:\\Program Files\\DSH Desktop Fork\\resources\\app.asar\\node_modules\\pnpm\\bin\\pnpm.mjs',
    electronVersion: '43.4.0',
    profileName: 'desktop',
    productVersion: '2.0.0',
...
describe('desktop terminal environment', () => {
  it('keeps generated shims isolated by active profile', () => {
    const desktop = desktopTerminalStateDirectory('/tmp/dsh-desktop-fork', 'desktop')
    const work = desktopTerminalStateDirectory('/tmp/dsh-desktop-fork', '工作 profile')

    expect(dirname(desktop)).toBe(join('/tmp/dsh-desktop-fork', 'cli'))
    expect(dirname(work)).toBe(join('/tmp/dsh-desktop-fork', 'cli'))
    expect(basename(desktop)).toMatch(/^[a-f0-9]{64}$/u)
    expect(basename(work)).toMatch(/^[a-f0-9]{64}$/u)
    expect(work).not.toBe(desktop)
    expect(desktopTerminalStateDirectory('/tmp/dsh-desktop-fork', 'desktop')).toBe(desktop)
  })
  ...
  it('generates private macOS shims and opens one quoted welcome command', () => {
    ...
    expect(dshShim).toContain("'/Applications/DSH O'\"'\"'Brien.app/Contents/MacOS/DSH Desktop Fork'")
    ...
    expect(nodeShim).toBe([
      '#!/bin/sh',
      `ELECTRON_RUN_AS_NODE=1 exec '/Applications/DSH O'"'"'Brien.app/Contents/MacOS/DSH Desktop Fork' "$@"`,
      '',
    ].join('\n'))
    ...
    expect(welcome).toContain('DSH Desktop Fork 2.0.0 terminal')
    ...
    expect(welcome).toContain('Restart DSH Desktop Fork after plugin changes.')
  ...
  it('generates Windows batch shims and opens PowerShell through a visible-console broker', () => {
    ...
    expect(welcome).toContain('"DSH Desktop Fork {0} terminal" -f $env:DSH_DESKTOP_PRODUCT_VERSION')
    ...
    expect(launcher).toContain('start "DSH Desktop Fork" /D "!DSH_DESKTOP_PROFILE_DIRECTORY!"')
  ...
  it('keeps Windows scripts ASCII and passes localized paths through the child environment', () => {
    ...
    options.appExecutable = 'C:\\程序\\DSH Desktop Fork.exe'
    ...
    expect(harness.calls[0]?.options.env).toEqual(expect.objectContaining({
      ...
      DSH_DESKTOP_APP_EXECUTABLE: 'C:\\程序\\DSH Desktop Fork.exe',
