...
describe('published package surface', () => {
  it('runs all desktop editions and community market typechecks from the root command', () => {
    // Root aggregates still target stable/beta; the fork has its own :fork entries.
  ...
  it('registers both npm launcher names', () => {
    expect(manifest.name).toBe('dsh-plugin-desktop-fork')
    expect(manifest.bin).toEqual({
      'dsh-plugin-desktop-fork': 'lib/bin.js',
      'dsh-desktop-fork': 'lib/bin.js',
    })
  })
  ...
  it('exposes the Host plugin and desktop-owned client face', () => {
    ...
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop-fork')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).not.toContain('name: dsh-community-market')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop-fork/terminal')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop-fork/pnpm')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop-fork/profiles')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop-fork/diagnostics')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop-fork/notifications')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop-fork/updates')
  })
  ...
  it('installs Host command PATHs after the launch snapshot and before profile boot', () => {
    ...
    expect(main).toContain("'dsh-plugin-desktop-fork: packaged pnpm runtime PATH'")
    expect(main).toContain("'dsh-plugin-desktop-fork: packaged dsh runtime PATH'")
  ...
  it('keeps active Profile preferences as the lazy source and serializes runtime mirrors', () => {
    ...
    const flushEffect = main.indexOf("'dsh-plugin-desktop-fork: flush Profile preference writes'", runtimeQueue)
  ...
  it('fixes the installed application identity', () => {
    ...
    expect(manifest.build?.productName).toBe('DSH Desktop Fork')
    expect(manifest.build?.appId).toBe('ai.deepseek.dsh.desktop.fork')
    ...
    // Fork points mac.icon at the hand-tinted .icns so the Dock icon is
    // distinguishable without an Xcode Composer re-export.
    expect(manifest.build?.mac?.icon).toBe('build/app-icon.icns')
    ...
    expect(manifest.build?.win?.artifactName).toBe('DSH-Desktop-Fork-${version}-${arch}-Portable.${ext}')
    expect(manifest.build?.nsis).toEqual({
      ...
      shortcutName: 'DSH Desktop Fork',
      uninstallerIcon: 'build/app-icon.ico',
      useZip: false,
      artifactName: 'DSH-Desktop-Fork-${version}-${arch}-Setup.${ext}',
    })
    expect(manifest.build?.linux?.icon).toBe('build/icons')
    expect(manifest.build?.linux?.synopsis).toBe('Agentic coding desktop for the DeepSeek Harness (Fork)')
  ...
  it('separates unsigned smoke packaging from the signed macOS release', () => {
    ...
    // Root release trains still target stable; the fork ships via :fork entries.
    ...
    expect(manifest.build?.mac?.artifactName).toBe('DSH-Desktop-Fork-${version}-${arch}.${ext}')
    expect(manifest.devDependencies?.['@electron/asar']).toBe('3.4.1')
    expect(manifest.devDependencies?.['@electron/fuses']).toBe('1.8.0')
  })

  it('exposes fork build and smoke-packaging entries from the root command', () => {
    expect(workspaceManifest.scripts?.['build:fork'])
      .toBe('yarn workspace dsh-plugin-desktop-fork build')
    expect(workspaceManifest.scripts?.['package:dir:fork'])
      .toBe('yarn aa:prepare-release && yarn workspace dsh-community-market build && yarn workspace dsh-plugin-desktop-fork package:dir')
    expect(workspaceManifest.scripts?.['dist:mac-smoke:fork'])
      .toBe('yarn aa:prepare-release && yarn workspace dsh-community-market build && yarn workspace dsh-plugin-desktop-fork dist:mac-smoke')
  })
  ...
  it('runs platform package gates before reusing native packaging outputs', () => {
    ...
    // CI still gates stable/beta only; the fork is a local-only variant.
  ...
  it('skips product packaging only for documentation-only changes', () => {
    ...
    expect(classify(['README.md', 'dsh-plugin-desktop-fork/src/index.ts'])).toBe('true')
