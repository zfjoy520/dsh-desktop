...
function options(calls: CommandCall[], logs: string[] = []): MacSmokePackageOptions {
  return {
    ...
    nodeVersion: '22.23.2',
    workspaceRoot: '/repo',
    desktopRoot: '/repo/dsh-plugin-desktop-fork',
    outputDir: '/repo/dsh-plugin-desktop-fork/dist/mac-smoke',
    resetOutput: () => undefined,
    prepareRuntime: () => undefined,
    builderCli: '/repo/node_modules/electron-builder/cli.js',
    verifier: '/repo/dsh-plugin-desktop-fork/scripts/verify-mac-smoke.ts',
...
describe('macOS DMG smoke packaging', () => {
  it('checks without credentials, builds an unsigned DMG, then verifies it', () => {
    ...
    expect(calls[0]).toEqual({
      command: 'corepack',
      args: ['yarn', 'workspace', 'dsh-plugin-desktop-fork', 'check:mac-package'],
      cwd: '/repo',
    ...
    expect(calls[1]).toEqual({
      ...
      args: [
        ...
        '--config.directories.output=/repo/dsh-plugin-desktop-fork/dist/mac-smoke',
      ],
      cwd: '/repo/dsh-plugin-desktop-fork',
      env: {
        PATH: '/usr/bin:/bin',
        SAFE_VALUE: 'kept',
    ...
    expect(calls[2]).toEqual({
      command: '/usr/local/bin/node',
      args: [
        '/repo/dsh-plugin-desktop-fork/scripts/verify-mac-smoke.ts',
        '/repo/dsh-plugin-desktop-fork/dist/mac-smoke',
      ],
      cwd: '/repo/dsh-plugin-desktop-fork',
      env: { PATH: '/usr/bin:/bin', SAFE_VALUE: 'kept' },
    })
