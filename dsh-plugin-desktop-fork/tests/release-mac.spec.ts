...
function baseOptions(
  ...
  return {
    ...
    outputDir: '/repo/dsh-plugin-desktop-fork/dist/mac-release',
...
describe('macOS release command boundary', () => {
  it('runs checks without credentials, then gives credentials only to the DMG builder', () => {
    ...
    expect(calls[0]).toEqual({
      ...
      cwd: resolve('/repo/dsh-plugin-desktop-fork', '..'),
    ...
    expect(calls[2]).toEqual({
      ...
      args: [
        ...
        '--config.directories.output=/repo/dsh-plugin-desktop-fork/dist/mac-release',
      ],
    ...
    expect(calls[3]).toEqual({
      ...
      args: [
        ...
        '/repo/dsh-plugin-desktop-fork/dist/mac-release',
      ],
      cwd: '/repo/dsh-plugin-desktop-fork',
      env: { PATH: '/usr/bin', SAFE_BUILD_VALUE: 'kept' },
    })
  ...
  it('does not clear output or build when AA freshness verification fails', () => {
    ...
    expect(calls[0]?.cwd).toBe(resolve('/repo/dsh-plugin-desktop-fork', '..'))
