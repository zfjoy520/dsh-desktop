...
describe('final Electron fuse verification', () => {
  it('maps only requested platform and architecture keys to complete runtime contexts', () => {
    const expected = [
      join('/build', 'linux-unpacked', 'dsh-plugin-desktop-fork'),
      join('/build', 'mac-universal', 'DSH Desktop Fork.app', 'Contents', 'MacOS', 'DSH Desktop Fork'),
      join('/build', 'win-unpacked', 'DSH Desktop Fork.exe'),
      join('/build', 'win-arm64-unpacked', 'DSH Desktop Fork.exe'),
    ].sort()
    ...
    expect(resolveFinalPackagedRuntimeContexts(
    ...
    )).toEqual([
      {
        ...
        packager: {
          executableName: 'dsh-plugin-desktop-fork',
          appInfo: { productFilename: 'DSH Desktop Fork' },
        },
      },
      {
        ...
        packager: { appInfo: { productFilename: 'DSH Desktop Fork' } },
      },
      {
  ...
  it('honors Linux executableName and recovers a configured suffixless architecture', () => {
    const configured = result([{ key: 'linux', archs: [Arch.arm64] }], {
      productName: 'DSH Desktop Fork',
      linux: { defaultArch: 'arm64', executableName: 'dsh-desktop-fork' },
    })
    const executable = join('/build', 'linux-unpacked', 'dsh-desktop-fork')
    ...
    expect(resolveFinalPackagedRuntimeContexts(
      ...
      electronPlatformName: 'linux',
      packager: {
        executableName: 'dsh-desktop-fork',
        appInfo: { productFilename: 'dsh-desktop-fork' },
      },
    }])
  })
  ...
  it('ignores a stale sibling architecture from an earlier build', () => {
    const expected = join('/build', 'win-unpacked', 'DSH Desktop Fork.exe')
    const stale = join('/build', 'win-arm64-unpacked', 'DSH Desktop Fork.exe')
  ...
  it('fails when one requested architecture is missing even if a sibling exists', () => {
    const x64Executable = join('/build', 'win-unpacked', 'DSH Desktop Fork.exe')

    expect(() => resolveFinalPackagedRuntimeContexts(
      result([{ key: 'win', archs: [Arch.x64, Arch.arm64] }]),
      filename => filename === x64Executable,
    )).toThrow(`win/arm64 at ${join('/build', 'win-arm64-unpacked', 'DSH Desktop Fork.exe')}`)
  })
  ...
  it('resolves a real target-name map through the target archs retained by NSIS', () => {
    ...
    const built = {
      outDir: '/build',
      configuration: { productName: 'DSH Desktop Fork' },
  ...
  it('checks every requested final executable after all artifact builds', async () => {
    const executables = [
      join('/build', 'mac-arm64', 'DSH Desktop Fork.app', 'Contents', 'MacOS', 'DSH Desktop Fork'),
      join('/build', 'mac', 'DSH Desktop Fork.app', 'Contents', 'MacOS', 'DSH Desktop Fork'),
    ]
    const events: string[] = []
  ...
  it.each([
    ...
    await expect(verifyElectronExecutableFuses('/build/DSH Desktop Fork.exe', read))
  ...
  it('wraps an unreadable final executable with its resolved path', async () => {
    ...
    await expect(verifyElectronExecutableFuses('/build/DSH Desktop Fork.exe', read))
      .rejects.toThrow('/build/DSH Desktop Fork.exe')
  })
})
