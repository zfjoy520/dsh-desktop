...
function temporaryHome(): string {
  const home = mkdtempSync(join(tmpdir(), 'dsh-desktop-fork-profile-'))
  homes.push(home)
  return home
}
      ...
      dsh: {
        profile: {
          bundles: [
            '@deepseek-ai/dsh-base',
            '@deepseek-ai/dsh-web-app',
            '@deepseek-ai/dsh-desktop-fork-app',
          ],
        },
      },
    }, undefined, 2) + '\n')
...
`)
  ...
  it('assembles the Host shell without replacing the upstream client shell', () => {
    ...
    expect(inserted).toContainEqual(expect.objectContaining({
      ...
      name: 'dsh-plugin-desktop-fork/webserver',
  ...
  it('reads JSON settings and defaults an absent desktop namespace to compatibility', () => {
    ...
    writeFileSync(path, JSON.stringify({ 'dsh-desktop-fork': { mode: 'advanced' } }))

    expect(readDesktopShellMode({ path })).toBe('advanced')
    expect(desktopStartupSettingsFromSettings({ 'dsh-desktop-fork': { mode: 'advanced', port: 43_189 } })).toEqual({
      mode: 'advanced',
      port: 43_189,
    ...
    expect(desktopStartupSettingsFromSettings({ 'dsh-desktop-fork': { mode: 'advanced' } })).toEqual({
      mode: 'advanced',
      port: 43_120,
      macosMaterial: 'transparent',
      windowsMaterial: 'off',
  ...
  it('rejects invalid settings roots, sections, modes, and YAML', () => {
    ...
    expect(() => desktopShellModeFromSettings({ 'dsh-desktop-fork': true })).toThrow('settings must be a map')
    expect(() => desktopShellModeFromSettings({ 'dsh-desktop-fork': { mode: 'glass' } })).toThrow(
      'must be "compatibility", "extended", or "advanced"',
    )
    for (const port of [-1, 1.5, 65_536, '43189']) {
      expect(() => desktopStartupSettingsFromSettings({ 'dsh-desktop-fork': { port } })).toThrow(
        'port must be an integer from 0 through 65535',
      )
    }
    expect(() => desktopStartupSettingsFromSettings({ 'dsh-desktop-fork': { openBrowser: 'yes' } }))
      .toThrow('openBrowser must be a boolean')
    expect(() => desktopStartupSettingsFromSettings({ 'dsh-desktop-fork': { networkExposure: 'internet' } }))
  ...
  it('keeps the Windows browse panel, official subprocess and presets, and desktop pwsh provider', () => {
    ...
    expect(rows).toContainEqual(expect.objectContaining({
      ...
      name: 'dsh-plugin-desktop-fork/windows-pwsh-sandbox',
