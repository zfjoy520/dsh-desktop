...
describe('native macOS application menu', () => {
  ...
  it('localizes the complete Simplified Chinese menu while retaining native roles', () => {
    const template = macApplicationMenuTemplate('DSH Desktop Fork', 'zh-CN')

    expect(template.map(item => item.label)).toEqual([
      'DSH Desktop Fork', '文件', '编辑', '显示', '窗口',
    ])
    expect(submenu(template[0]!).map(item => item.label).filter(Boolean)).toEqual([
      '关于 DSH Desktop Fork', '服务', '隐藏 DSH Desktop Fork', '隐藏其他', '全部显示', '退出 DSH Desktop Fork',
    ])
  ...
  it('keeps the English fallback complete', () => {
    const template = macApplicationMenuTemplate('DSH Desktop Fork', 'en')

    expect(template.map(item => item.label)).toEqual([
      'DSH Desktop Fork', 'File', 'Edit', 'View', 'Window',
    ])
    expect(submenu(template[0]!)).toEqual(expect.arrayContaining([
      expect.objectContaining({ label: 'About DSH Desktop Fork', role: 'about' }),
      expect.objectContaining({ label: 'Quit DSH Desktop Fork', role: 'quit' }),
    ]))
  })
