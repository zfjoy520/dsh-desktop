...
describe('packaged desktop runtime verification', () => {
  ...
  it('recursively derives every non-map desktop runtime file from the completed build', () => {
    const root = mkdtempSync(join(tmpdir(), 'dsh-desktop-fork-runtime-list-'))
  ...
  it.each([
    [
      'darwin',
      join('/build', 'DSH Desktop Fork.app', 'Contents', 'Resources', 'app.asar'),
      join('/build', 'DSH Desktop Fork.app', 'Contents', 'MacOS', 'DSH Desktop Fork'),
    ],
    [
      'win32',
      join('/build', 'resources', 'app.asar'),
      join('/build', 'DSH Desktop Fork.exe'),
    ],
  ...
  it('uses LinuxPackager executableName instead of appInfo.productFilename', () => {
    const runtimeContext = context('/build', 'linux', 1, 'dsh-plugin-desktop-fork')
    ...
    expect(resolvePackagedExecutablePath(runtimeContext))
      .toBe(join('/build', 'dsh-plugin-desktop-fork'))
  })
  ...
  it('runs every logical ASAR entry through the packaged Electron executable', () => {
    const runtimeContext = context(
      ...
      process.platform === 'linux' ? 'dsh-plugin-desktop-fork' : undefined,
    )
