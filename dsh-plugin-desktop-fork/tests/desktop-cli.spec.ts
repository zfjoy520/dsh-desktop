...
describe('packaged dsh bootstrap', () => {
  ...
  it('clears Node mode and dispatches the imported CLI exactly once', async () => {
    ...
    const argv = ['/Applications/DSH Desktop Fork', '/app.asar/lib/desktop-cli.js', '--dump-config']
    const runCli = vi.fn(async () => {})
    const load = vi.fn(async (url: string) => {
      expect(environment).toEqual({ KEEP: 'value' })
      expect(argv).toEqual([
        '/Applications/DSH Desktop Fork',
  ...
  it('uses the physical unpacked dependency tree only inside an Electron package', () => {
    expect(unpackedAsarPath('/Applications/DSH Desktop Fork.app/Contents/Resources/app.asar/node_modules/pkg'))
      .toBe('/Applications/DSH Desktop Fork.app/Contents/Resources/app.asar.unpacked/node_modules/pkg')
    expect(unpackedAsarPath('C:\\Program Files\\DSH Desktop Fork\\resources\\app.asar\\node_modules\\pkg'))
      .toBe('C:\\Program Files\\DSH Desktop Fork\\resources\\app.asar.unpacked\\node_modules\\pkg')
    expect(unpackedAsarPath('/Applications/DSH Desktop Fork.app/Contents/Resources/app.asar/package.json'))
      .toBe('/Applications/DSH Desktop Fork.app/Contents/Resources/app.asar.unpacked/package.json')
  ...
  it('keeps a resolved JavaScript dependency in the logical ASAR tree', () => {
    const root = mkdtempSync(join(tmpdir(), 'dsh-desktop-fork-asar-profile-'))
  ...
  it('resolves the DSH entry from a pnpm profile with flat package dependencies', () => {
    const root = mkdtempSync(join(tmpdir(), 'dsh-desktop-fork-flat-profile-'))
    const desktopLib = join(root, 'node_modules', 'dsh-plugin-desktop-fork', 'lib')
