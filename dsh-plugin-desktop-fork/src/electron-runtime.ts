...
export function desktopProductVersion(moduleUrl: string = import.meta.url): string {
  ...
  if (value === null || typeof value !== 'object' || typeof (value as { version?: unknown }).version !== 'string') {
    throw new Error('dsh-plugin-desktop-fork: package.json has no product version')
  }
...
export function requestDesktopArtifact(url: string, init: RequestInit): Promise<UpdateArtifactResponse> {
  return new Promise((resolve, reject) => {
    ...
    request.on('response', incoming => {
      ...
      if (status === undefined) {
        reject(new Error('dsh-plugin-desktop-fork: the update download response carried no HTTP status.'))
        return
      }
...
/** Native adapter used by the DSH Desktop Fork launcher and owned by its Cordis shell plugin. */
export class ElectronDesktopRuntime implements DesktopRuntime {
  ...
  beginRendererBootMonitoring(
    ...
    if (this.rendererHealthGate !== undefined) {
      throw new Error('dsh-plugin-desktop-fork: renderer boot monitoring already started')
    }
  ...
  schedule(spec: DesktopShellSpec): () => Promise<void> {
    if (this.scheduled !== undefined || this.mountTask !== undefined) {
      throw new Error('dsh-plugin-desktop-fork: a native shell generation is already registered')
    }
  ...
  mountScheduled(beforeInteractive?: () => void): Promise<void> {
    ...
    if (spec === undefined) {
      return Promise.reject(new Error('dsh-plugin-desktop-fork: the Cordis shell plugin did not register a window'))
    }
    if (this.mountTask === undefined) {
      ...
      this.mountTask = generation.mount(beforeInteractive).then(() => {
        ...
        void this.offerUpdateArtifactCleanup().catch((cause: unknown) => {
          this.logError(`dsh-plugin-desktop-fork: failed to resolve update installer cleanup: ${cause instanceof Error ? cause.message : String(cause)}`)
        })
      }).catch((cause: unknown) => {
  ...
  configureTerminal(spec: DesktopTerminalSpec): void {
    if (this.terminalSpec !== undefined) {
      throw new Error('dsh-plugin-desktop-fork: terminal profile is already configured')
    }
  ...
  openTerminal(): void {
    try {
      ...
      if (spec === undefined) {
        throw new Error('dsh-plugin-desktop-fork: terminal profile is not configured')
      }
      const electronVersion = process.versions.electron
      if (electronVersion === undefined) {
        throw new Error('dsh-plugin-desktop-fork: terminal requires the Electron runtime version')
      }
      openDesktopTerminal({
        platform: this.platform,
  ...
  reloadRenderer(): void {
    if (this.generation === undefined) {
      throw new Error('dsh-plugin-desktop-fork: renderer reload requires an active shell generation')
    }
  ...
  toggleDeveloperTools(): void {
    if (this.generation === undefined) {
      throw new Error('dsh-plugin-desktop-fork: Developer Tools require an active shell generation')
    }
  ...
  private handleRendererBootVerdict(report: RendererBootReport): void {
    ...
    if (report.status === 'failed') {
      ...
      this.logError(`dsh-plugin-desktop-fork: renderer boot failed (plugins: ${plugins}): ${error}`)
    }
    let handled = false
    try {
      handled = this.onRendererBoot(report) === true
    } catch (cause) {
      this.logError(`dsh-plugin-desktop-fork: failed to persist renderer boot health: ${cause instanceof Error ? cause.message : String(cause)}`)
    }
    if (report.status === 'failed' && !handled) {
      void this.showRendererBootRecovery(report).catch((cause: unknown) => {
        this.logError(`dsh-plugin-desktop-fork: failed to show plugin recovery: ${cause instanceof Error ? cause.message : String(cause)}`)
      })
    }
  }
  ...
  private trayCommand(invoke: () => void | Promise<void>): () => void {
    return () => {
      void Promise.resolve().then(invoke).catch((cause: unknown) => {
        this.logError(`dsh-plugin-desktop-fork: tray command failed: ${cause instanceof Error ? cause.message : String(cause)}`)
      })
    }
  }
  ...
  private async downloadAndOpenUpdate(
    ...
    if (platform === undefined) {
      throw new Error(`dsh-plugin-desktop-fork: updates are unavailable on ${this.platform}`)
    }
    ...
    try {
      ...
      this.logError(`dsh-plugin-desktop-fork: failed to remember update installer for cleanup: ${cause instanceof Error ? cause.message : String(cause)}`)
    }

    if (platform === 'darwin') {
      const openError = await shell.openPath(artifactPath)
      if (openError !== '') throw new Error(`dsh-plugin-desktop-fork: failed to open update disk image: ${openError}`)
      signal.throwIfAborted()
      await this.showUpdateMessageBox({
        type: 'info',
    ...
    const spec = this.scheduled
    if (spec === undefined) throw new Error('dsh-plugin-desktop-fork: no active shell can exit for update installation')
    signal.throwIfAborted()
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.once('spawn', () => {
        ...
        child.once('error', cause => {
          this.logError(`dsh-plugin-desktop-fork: update installer failed after launch: ${cause.message}`)
        })
        child.unref()
        resolve()
      })
  ...
  private reportTerminalLaunchError(cause: unknown): void {
    ...
    const copy = desktopNativeCopy(this.currentLocale)
    this.logError(`dsh-plugin-desktop-fork: failed to open terminal: ${error.message}`)
    void this.showDesktopMessageBox({
      ...
      this.logError(`dsh-plugin-desktop-fork: failed to show terminal error: ${dialogCause instanceof Error ? dialogCause.message : String(dialogCause)}`)
    })
  }
  ...
  private reportDiagnosticExportError(cause: unknown): void {
    const error = cause instanceof Error ? cause : new Error(String(cause))
    const copy = desktopNativeCopy(this.currentLocale)
    this.logError(`dsh-plugin-desktop-fork: failed to export diagnostics: ${error.message}`)
    void this.showDesktopMessageBox({
      type: 'error',
      ...
      this.logError(`dsh-plugin-desktop-fork: failed to show diagnostics error: ${dialogCause instanceof Error ? dialogCause.message : String(dialogCause)}`)
    })
  }
  ...
  private buildTrayTemplate(spec: DesktopShellSpec): Electron.MenuItemConstructorOptions[] {
    ...
    const changeMode = (mode: DesktopShellSpec['mode']): void => {
      ...
      void spec.requestModeChange(mode).catch((cause: unknown) => {
        this.logError(`dsh-plugin-desktop-fork: failed to change shell mode: ${cause instanceof Error ? cause.message : String(cause)}`)
      })
    }
    ...
    const reloadRenderer = (): void => {
      try {
        ...
        this.logError(`dsh-plugin-desktop-fork: failed to reload the renderer from the tray: ${cause instanceof Error ? cause.message : String(cause)}`)
      }
    }
