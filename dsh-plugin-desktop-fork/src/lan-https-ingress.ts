...
function exactPort(value: number, allowZero: boolean, label: string): number {
  ...
  if (!Number.isSafeInteger(value) || value < minimum || value > 65_535) {
    throw new TypeError(`dsh-plugin-desktop-fork: ${label} must be an integer from ${String(minimum)} through 65535`)
  }
  return value
}
...
function canonicalIpv4(value: string): string {
  if (!isIPv4(value)) {
    throw new TypeError(`dsh-plugin-desktop-fork: invalid LAN HTTPS IPv4 address ${JSON.stringify(value)}`)
  }
  const canonical = value.split('.').map(part => String(Number(part))).join('.')
  if (canonical !== value) {
    throw new TypeError(`dsh-plugin-desktop-fork: LAN HTTPS IPv4 address must be canonical: ${JSON.stringify(value)}`)
  }
  return canonical
}
...
function copyTlsValue(value: string | Buffer, label: string): string | Buffer {
  if ((typeof value === 'string' && value.length === 0) || (Buffer.isBuffer(value) && value.length === 0)) {
    throw new TypeError(`dsh-plugin-desktop-fork: LAN HTTPS ${label} must not be empty`)
  }
...
export class LanHttpsIngress {
  ...
  constructor(options: LanHttpsIngressOptions) {
    ...
    if (fingerprint.length === 0) throw new TypeError('dsh-plugin-desktop-fork: LAN HTTPS CA fingerprint must not be empty')
