      ...
      if (response.ok) {
        ...
        if (sessionId !== undefined) {
          try {
            ...
            console.error('dsh-plugin-desktop-fork: failed to open jumped session', cause)
          }
        }
      }
    } catch (cause) {
      console.error('dsh-plugin-desktop-fork: failed to poll the session jump route', cause)
    } finally {
