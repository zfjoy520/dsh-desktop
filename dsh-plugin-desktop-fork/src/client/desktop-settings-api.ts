...
const LAN_CA_PATH = '/.well-known/dsh-desktop-fork-ca.crt'
...
function parseProfile(value: unknown): DesktopProfileView {
  if (!isObject(value)
    ...
    throw new Error('dsh-plugin-desktop-fork: invalid profile settings response')
  }
...
function parseBrowserUrl(value: unknown, loopback: boolean): string {
  if (typeof value !== 'string' || value.length > 2_048) {
    throw new Error('dsh-plugin-desktop-fork: invalid browser URL in settings response')
  }
  let url: URL
  try {
    url = new URL(value)
...
function parseLanCaUrl(value: unknown): string {
  if (typeof value !== 'string' || value.length > 2_048) {
    throw new Error('dsh-plugin-desktop-fork: invalid LAN CA URL in settings response')
  }
  let url: URL
  try {
    url = new URL(value)
...
function parseLanError(value: unknown): string | null {
  ...
  if (typeof value !== 'string' || value.length === 0 || value.length > MAX_LAN_ERROR_LENGTH
    ...
    throw new Error('dsh-plugin-desktop-fork: invalid LAN HTTPS error in settings response')
  }
  return value
}
...
function parseLanCaFingerprint(value: unknown): string | null {
  ...
  if (typeof value !== 'string' || !SHA256_FINGERPRINT_PATTERN.test(value)) {
    throw new Error('dsh-plugin-desktop-fork: invalid LAN CA fingerprint in settings response')
  }
  return value
}
...
export function parseDesktopSettingsView(value: unknown): DesktopSettingsView {
  if (!isObject(value)
    ...
    throw new Error('dsh-plugin-desktop-fork: invalid Desktop settings response')
  }
  ...
  if (new Set(profiles.map(profile => profile.name)).size !== profiles.length) {
    throw new Error('dsh-plugin-desktop-fork: duplicate profile in settings response')
  }
  if (new Set(lanUrls).size !== lanUrls.length || new Set(lanCaUrls).size !== lanCaUrls.length) {
    throw new Error('dsh-plugin-desktop-fork: duplicate LAN URL in settings response')
  }
  if ((value.web.lanState === 'ready') !== (lanUrls.length > 0)) {
    throw new Error('dsh-plugin-desktop-fork: inconsistent LAN HTTPS state in settings response')
  }
  if (value.web.lanState !== 'failed' && lanError !== null) {
    throw new Error('dsh-plugin-desktop-fork: inconsistent LAN HTTPS error in settings response')
  }
  return Object.freeze({
...
export function parseDesktopRestartAcceptance(value: unknown): DesktopRestartAcceptance {
  if (!isObject(value) || value.accepted !== true || typeof value.restartRequired !== 'boolean') {
    throw new Error('dsh-plugin-desktop-fork: invalid Desktop restart response')
  }
...
export function parseDesktopActionAcceptance(value: unknown): void {
  if (!isObject(value)
    ...
    || value.accepted !== true) {
    throw new Error('dsh-plugin-desktop-fork: invalid Desktop action response')
  }
}

async function readResponse(response: Response): Promise<unknown> {
  if (!response.ok) {
    throw new Error(`dsh-plugin-desktop-fork: Desktop settings request failed (${String(response.status)})`)
  }
  try {
    return await response.json() as unknown
  } catch {
    throw new Error('dsh-plugin-desktop-fork: Desktop settings response was not JSON')
  }
}
