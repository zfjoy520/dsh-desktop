...
export function parseDesktopClientEnvironment(search: string): DesktopClientEnvironment | undefined {
  ...
  const mode = params.get('dsh-desktop-fork-mode')
  const platform = params.get('dsh-desktop-fork-platform')
  const materialMarker = params.get('dsh-desktop-fork-material')
  const version = params.get('dsh-desktop-fork-version')
  if (mode === null && platform === null) return undefined
  if (!MODES.has(mode as DesktopClientMode)) {
    throw new Error(`dsh-plugin-desktop-fork: invalid or missing dsh-desktop-fork-mode ${JSON.stringify(mode)}`)
  }
  if (!PLATFORMS.has(platform as DesktopClientPlatform)) {
    throw new Error(`dsh-plugin-desktop-fork: invalid or missing dsh-desktop-fork-platform ${JSON.stringify(platform)}`)
  }
  if (!MATERIAL_MARKERS.has(materialMarker ?? '')) {
    throw new Error(`dsh-plugin-desktop-fork: invalid or missing dsh-desktop-fork-material ${JSON.stringify(materialMarker)}`)
  }
  ...
  if (version === null || version.length > 64 || !VERSION_PATTERN.test(version)) {
    throw new Error(`dsh-plugin-desktop-fork: invalid or missing dsh-desktop-fork-version ${JSON.stringify(version)}`)
  }
  const micaMarker = params.get('dsh-desktop-fork-mica')
  ...
  if (micaSupported === undefined) {
    throw new Error(`dsh-plugin-desktop-fork: invalid dsh-desktop-fork-mica ${JSON.stringify(micaMarker)}`)
  }
  if ((platform === 'darwin' && materialMarker !== 'off' && materialMarker !== 'transparent')
    ...
    throw new Error('dsh-plugin-desktop-fork: renderer material is incompatible with its mode or platform')
  }
  return {
    version,
