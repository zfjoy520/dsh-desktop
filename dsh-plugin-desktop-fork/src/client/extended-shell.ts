...
export function applyFramedShell(
  ...
  if (environment.mode !== 'compatibility' && environment.mode !== 'extended') {
    throw new Error(`dsh-plugin-desktop-fork: framed shell received mode ${JSON.stringify(environment.mode)}`)
  }
  ctx.effect(() => {
    const contentViewport = document.getElementById('root')
    if (contentViewport === null) {
      throw new Error('dsh-plugin-desktop-fork: framed shell requires the upstream root')
    }
...
export function applyExtendedShell(
  ...
  if (environment.mode !== 'extended') {
    throw new Error(`dsh-plugin-desktop-fork: extended shell received mode ${JSON.stringify(environment.mode)}`)
  }
