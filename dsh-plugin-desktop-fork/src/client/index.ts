...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(
    ...
    'dsh-plugin-desktop-fork: native window geometry service',
  )
  ...
  ctx.effect(
    ...
    'dsh-plugin-desktop-fork: sidebar footer stacking styles',
  )
  ctx.effect(
    () => startRendererBootReporter(ctx.loader),
    'dsh-plugin-desktop-fork: renderer boot health report',
  )
  ctx.effect(
    () => startDesktopSessionJumpPoll(ctx as unknown as { sessions: DesktopSessionJumpReader }),
    'dsh-plugin-desktop-fork: Dock session jump poll',
  )
  if (environment.platform === 'win32') {
    ctx.effect(
      () => installDesktopDirectoryPickerBridge(),
      'dsh-plugin-desktop-fork: native directory picker bridge',
    )
  }
