/** DSH Desktop Fork Host plugin: owns the selected native shell generation. */
...
export const DESKTOP_SETTINGS_NAMESPACE = 'dsh-desktop-fork'
...
export function desktopRendererUrl(
  ...
  url.searchParams.set('dsh-desktop-fork-mode', mode)
  url.searchParams.set('dsh-desktop-fork-platform', platform)
  url.searchParams.set('dsh-desktop-fork-version', appVersion)
  url.searchParams.set('dsh-desktop-fork-material', material)
  if (mode === 'extended' || (mode === 'compatibility' && platform !== 'linux')) {
    ...
    url.searchParams.set('dsh-desktop-fork-titlebar-inset', String(DESKTOP_FRAME_HEIGHT))
  }
  if (platform === 'win32') {
    url.searchParams.set('dsh-desktop-fork-mica', windowsSupportsMica(windowsBuild) ? '1' : '0')
  }
  return url.href
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (runtime === undefined) {
    process.stderr.write(
      'dsh-plugin-desktop-fork: this profile is composed with the DSH Desktop Fork shell, which requires the desktop launcher (desktopRuntime).\n'
      + 'Start it with `dsh-desktop-fork`, or select this profile inside the packaged DSH Desktop Fork application.\n'
  ...
  if (appExit === undefined) {
    throw new Error('dsh-plugin-desktop-fork: the launcher did not provide ctx.appExit')
  }
  const browserAccess = ctx.get('desktopBrowserAccess')
  if (browserAccess === undefined) {
    throw new Error('dsh-plugin-desktop-fork: the launcher did not provide ctx.desktopBrowserAccess')
  }
  const lanHttps = ctx.get('desktopLanHttps')
  if (lanHttps === undefined) {
    throw new Error('dsh-plugin-desktop-fork: the launcher did not provide ctx.desktopLanHttps')
  }
  if (ctx.webServer.host !== desktopWebServerHost(config.networkExposure)) {
    throw new Error('dsh-plugin-desktop-fork: desktop shell WebServer host does not match networkExposure')
  }
    ...
    DesktopSettingsSchema,
    {
      ...
      validate: (value) => {
        if (!desktopBrowserAccessAvailable(value.mode) && value.openBrowser) {
          throw new Error('dsh-plugin-desktop-fork: browser access requires compatibility mode')
        }
        if (value.mode !== 'compatibility' && runtime.platform === 'linux') {
          throw new Error('dsh-plugin-desktop-fork: custom desktop shell modes are supported on macOS and Windows')
        }
      },
    },
  )
  ...
  ctx.effect(
    () => ctx.webServer.register({
      ...
      handler: (req, res) => {
        ...
        res.setHeader('content-disposition', 'attachment; filename="dsh-desktop-fork-local-ca.crt"')
    ...
    'dsh-plugin-desktop-fork: public LAN HTTPS CA route',
  )
  ...
  if (desktopSettings !== undefined) {
    const reportSettingsError = (operation: string, cause: unknown): void => {
      ctx.logger.error(
        `dsh-plugin-desktop-fork: failed to ${operation}: ${cause instanceof Error ? cause.message : String(cause)}`,
      )
    }
    ...
    for (const [path, handler] of settingsRoutes) {
      ctx.effect(
        ...
        `dsh-plugin-desktop-fork: private settings route ${path}`,
      )
    }
    ...
    ctx.effect(
      ...
      `dsh-plugin-desktop-fork: private settings route ${DESKTOP_SESSION_JUMP_PATH}`,
    )
  }
  ctx.effect(
    () => ctx.webServer.register({
    ...
    'dsh-plugin-desktop-fork: renderer boot report route',
  )
      ...
      () => ctx.webServer.register({
        ...
        handler: (req, res) => {
          ...
          return handleDesktopDirectoryPickerRequest(
            ...
            cause => {
              ctx.logger.error(`dsh-plugin-desktop-fork: native directory picker failed: ${cause instanceof Error ? cause.message : String(cause)}`)
            },
          )
        },
      }),
      'dsh-plugin-desktop-fork: native directory picker route',
    )
    ctx.effect(
      () => ctx.webServer.register({
        kind: 'exact',
        ...
        handler: (req, res) => {
          ...
          return handleDesktopDirectoryValidationRequest(
            ...
            cause => {
              ctx.logger.error(`dsh-plugin-desktop-fork: workspace directory validation failed: ${cause instanceof Error ? cause.message : String(cause)}`)
            },
          )
        },
      }),
      'dsh-plugin-desktop-fork: workspace directory validation route',
    )
  }
  ctx.effect(() => {
    ...
    const updateLiveWebAccess = (
      ...
      void lanHttps.setEnabled(browserEnabled && exposure === 'lan').then((snapshot) => {
        if (snapshot.state === 'failed') {
          ctx.logger.error(
            `dsh-plugin-desktop-fork: LAN HTTPS edge failed to start (${snapshot.errorCode ?? 'unknown'})`,
          )
        }
      }).catch((cause: unknown) => {
        ctx.logger.error(
          `dsh-plugin-desktop-fork: LAN HTTPS edge transition failed: ${cause instanceof Error ? cause.message : String(cause)}`,
        )
      })
    }
    ...
    const stopWatching = settings.watch((next) => {
      ...
      pending ??= setImmediate(() => {
        ...
        void runtime.requestRestart().catch((cause: unknown) => {
          ctx.logger.error('dsh-plugin-desktop-fork: failed to restart after startup setting change')
  ...
  }, 'dsh-plugin-desktop-fork: live browser access and restart-applied native settings')
  ...
  ctx.effect(
    () => {
      ...
      return runtime.schedule({
        ...
        readThemeSource: () => {
          ...
          if (theme === undefined) {
            throw new Error('dsh-plugin-desktop-fork: custom shell requires the ui-theme settings namespace')
          }
    ...
    'dsh-plugin-desktop-fork: native shell generation',
  )
}
