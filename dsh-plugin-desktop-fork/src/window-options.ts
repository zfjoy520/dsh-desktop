...
export const DESKTOP_RENDERER_SESSION_PARTITION = 'persist:dsh-desktop-fork-renderer'
...
export function compatibilityWindowOptions(
  ...
  if (spec.mode !== 'compatibility') {
    throw new Error(`dsh-plugin-desktop-fork: unsupported compatibility window mode ${spec.mode}`)
  }
  ...
  throw new Error('dsh-plugin-desktop-fork: compatibility mode is unsupported on this platform')
}
...
export function advancedWindowOptions(
  ...
  if (spec.mode !== 'advanced') {
    throw new Error(`dsh-plugin-desktop-fork: unsupported enhanced window mode ${spec.mode}`)
  }
  return customChromeWindowOptions(spec, icon, platform, preload, {
...
export function extendedWindowOptions(
  ...
  if (spec.mode !== 'extended') {
    throw new Error(`dsh-plugin-desktop-fork: unsupported extended window mode ${spec.mode}`)
  }
  return customChromeWindowOptions(spec, icon, platform, preload, {
    titlebarHeight: DESKTOP_FRAME_HEIGHT,
    macosTrafficLightTop: DESKTOP_FRAME_MACOS_TRAFFIC_LIGHT_TOP,
