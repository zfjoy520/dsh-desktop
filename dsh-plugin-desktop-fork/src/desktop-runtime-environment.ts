...
function assertScriptValue(label: string, value: string): void {
  if (value.length === 0) {
    throw new Error(`dsh-plugin-desktop-fork: command runtime ${label} must not be empty`)
  }
  if (/[\0\r\n]/u.test(value)) {
    throw new Error(`dsh-plugin-desktop-fork: command runtime ${label} must not contain NUL or newlines`)
  }
}
...
function quoteBatchWord(value: string): string {
  if (/["\r\n]/u.test(value)) {
    throw new Error('dsh-plugin-desktop-fork: pnpm runtime Windows arguments must not contain quotes or newlines')
  }
  return `"${value.replaceAll('%', '%%')}"`
}
...
function escapeBatchSetValue(value: string): string {
  if (/["\r\n]/u.test(value)) {
    throw new Error('dsh-plugin-desktop-fork: pnpm runtime Windows environment values must not contain quotes or newlines')
  }
  return value.replaceAll('%', '%%')
}
...
function preparePrivateDirectory(directory: string): void {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    throw new Error(`dsh-plugin-desktop-fork: command runtime path is not a private directory: ${directory}`)
  }
...
function reportCleanupFailure(filename: string, cause: unknown): void {
  ...
  try {
    process.stderr.write(
      `dsh-plugin-desktop-fork: deferred command runtime cleanup for ${JSON.stringify(filename)}`
        + `${code === undefined ? '' : ` (${code})`}\n`,
    )
  } catch {
...
export function installDesktopDshRuntime(options: DesktopDshRuntimeOptions): DesktopDshRuntimeInstallation {
  if (options.platform !== 'win32') {
    throw new Error(`dsh-plugin-desktop-fork: dsh runtime is unsupported on ${options.platform}`)
  }
...
export function installDesktopPnpmRuntime(options: DesktopPnpmRuntimeOptions): DesktopPnpmRuntimeInstallation {
  if (options.platform !== 'darwin' && options.platform !== 'linux' && options.platform !== 'win32') {
    throw new Error(`dsh-plugin-desktop-fork: pnpm runtime is unsupported on ${options.platform}`)
  }
  for (const [label, value] of [
    ['application executable', options.appExecutable],
