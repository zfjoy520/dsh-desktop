...
function pairedWebSocketOrigin(origin: string): string {
  ...
  else throw new Error(`dsh-plugin-desktop-fork: unsupported renderer origin protocol ${url.protocol}`)
  return url.origin
}
...
async function authenticateRendererSession(
  ...
  if (authenticated.status !== 200) {
    throw new Error(
      `dsh-plugin-desktop-fork: browser authentication failed with HTTP ${String(authenticated.status)}`,
    )
  }
...
export class ElectronShellGeneration {
  ...
  constructor(private readonly options: ElectronShellGenerationOptions) {
    this.rendererRecovery = new DesktopRendererRecovery({
      ...
      log: message => { this.options.logError(`dsh-plugin-desktop-fork: ${message}`) },
    ...
    this.surfaceWatchdog = new RendererSurfaceWatchdog({
      ...
      failed: (detail, unresponsive) => {
        this.options.logError(`dsh-plugin-desktop-fork: ${detail}`)
  ...
  async mount(beforeInteractive?: () => void): Promise<void> {
    if (this.mounted || this.window !== undefined) {
      throw new Error('dsh-plugin-desktop-fork: native shell generation is already mounted')
    }

    const { platform, spec } = this.options
    const icon = nativeImage.createFromPath(spec.iconPath)
    if (icon.isEmpty()) {
      throw new Error(`dsh-plugin-desktop-fork: failed to load application icon ${spec.iconPath}`)
    }
    ...
    try {
      ...
      this.options.logError(`dsh-plugin-desktop-fork: failed to restore main-window state: ${cause instanceof Error ? cause.message : String(cause)}`)
    }
    ...
    renderer.ipc.handle(DESKTOP_RENDERER_ACTION_CHANNEL, async (event, action: unknown) => {
      if (this.released || event.sender !== renderer
        ...
        throw new Error('dsh-plugin-desktop-fork: untrusted Desktop action sender')
      }
    ...
    const persistWindowState = (): void => {
      ...
      try {
        ...
        this.options.logError(`dsh-plugin-desktop-fork: failed to save main-window state: ${cause instanceof Error ? cause.message : String(cause)}`)
      }
    }
    ...
    const loadFailed = (
      ...
      this.options.logError(`dsh-plugin-desktop-fork: renderer failed to load (${errorCode}: ${errorDescription})`)
      ...
      try {
        ...
        if (target.protocol === 'https:' || target.protocol === 'http:' || target.protocol === 'mailto:') {
          void shell.openExternal(target.href).catch((cause: unknown) => {
            this.options.logError(`dsh-plugin-desktop-fork: failed to open external link: ${cause instanceof Error ? cause.message : String(cause)}`)
          })
        }
      } catch {
  ...
  private async offerRendererRecovery(): Promise<void> {
    ...
    try {
      ...
      this.options.logError(`dsh-plugin-desktop-fork: renderer recovery failed: ${cause instanceof Error ? cause.message : String(cause)}`)
    } finally {
  ...
  reloadRenderer(): void {
    ...
    if (window === undefined || window.isDestroyed()) {
      throw new Error('dsh-plugin-desktop-fork: renderer reload requires a mounted window')
    }
  ...
  private beginRendererReplacement(): boolean {
    ...
    this.replacementExit = setTimeout(() => {
      ...
      this.options.logError('dsh-plugin-desktop-fork: forced renderer termination reported no exit within the deadline; reloading anyway')
      this.renderer?.reloadIgnoringCache()
    ...
    this.options.logError('dsh-plugin-desktop-fork: terminating unresponsive renderer; the crash dump it produces is deliberate')
  ...
  private finishRendererReplacement(details: Electron.RenderProcessGoneDetails): void {
    ...
    this.options.logError(
      `dsh-plugin-desktop-fork: unresponsive renderer replaced (reason: ${details.reason}, exitCode: ${formatDesktopExitCode(details.exitCode)})`,
    )
    this.renderer?.reloadIgnoringCache()
  }
  ...
  toggleDeveloperTools(): void {
    const window = this.window
    if (window === undefined || window.isDestroyed()) {
      throw new Error('dsh-plugin-desktop-fork: Developer Tools require a mounted window')
    }
    const renderer = this.renderer
