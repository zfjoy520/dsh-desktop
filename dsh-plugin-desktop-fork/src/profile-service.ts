...
export class DesktopProfileService extends Service implements DesktopProfiles {
  ...
  constructor(ctx: Context, private readonly bootstrap: DesktopProfileServiceBootstrap) {
    ...
    ctx.effect(
      () => () => { this.disposed = true },
      'dsh-plugin-desktop-fork: profile service lifetime',
    )
  }
  ...
  create(name: string): DesktopProfileSummary {
    ...
    if (this.bootstrap.create === undefined) {
      throw new Error('dsh-plugin-desktop-fork: profile creation is unavailable')
    }
    ...
    try {
      ...
      if (this.committedName !== undefined) {
        ...
        if (this.committedSelection === undefined) {
          return Promise.reject(new Error('dsh-plugin-desktop-fork: committed Profile selection is unavailable'))
        }
  ...
  async delete(name: string): Promise<void> {
    ...
    if (this.isSelectionTarget(name)) {
      throw new Error(`dsh-plugin-desktop-fork: selected profile ${JSON.stringify(name)} cannot be deleted`)
    }
    if (this.bootstrap.delete === undefined) {
      throw new Error('dsh-plugin-desktop-fork: profile deletion is unavailable')
    }
  ...
  private committedSelectionError(name: string): Error {
    return new Error(
      `dsh-plugin-desktop-fork: profile ${JSON.stringify(this.committedName)} is already selected for restart; `
  ...
  private assertActive(): void {
    if (this.disposed) throw new Error('dsh-plugin-desktop-fork: desktopProfiles service disposed')
  }
}
