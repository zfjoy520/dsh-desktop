/** DSH Desktop Fork executable: minimal Electron bootstrap around the Host Cordis root. */
...
async function start(): Promise<void> {
  ...
  runtime = new ElectronDesktopRuntime(async target => {
    if (shutdown === undefined) {
      throw new Error('dsh-plugin-desktop-fork: shutdown coordinator is not ready')
    }
      ...
      const ctx = await boot(
        ...
        async (hostCtx) => {
          ...
          hostCtx.effect(
            ...
            'dsh-plugin-desktop-fork: flush Profile preference writes',
          )
          hostCtx.effect(
            () => releasePnpmRuntime,
            'dsh-plugin-desktop-fork: packaged pnpm runtime PATH',
          )
          if (dshRuntime !== undefined) {
            hostCtx.effect(
              () => releaseDshRuntime,
              'dsh-plugin-desktop-fork: packaged dsh runtime PATH',
            )
          }
          hostCtx.effect(
            () => releasePackageResolver,
            'dsh-plugin-desktop-fork: profile package resolution',
          )
          ...
          hostCtx.effect(() => () => {
            ...
            pendingSettingsRestart = undefined
          }, 'dsh-plugin-desktop-fork: pending Desktop settings restart')
...
async function run(): Promise<void> {
  ...
  if (process.argv.includes('--export-diagnostics')) {
    try {
      ...
      const message = `dsh-plugin-desktop-fork: failed to export diagnostics: ${cause instanceof Error ? cause.stack ?? cause.message : String(cause)}\n`
      await new Promise<void>(resolve => {
