/** Headless version checks against the public DSH Desktop Fork release service. */
...
/** Public endpoint returning the latest DSH Desktop Fork version for a requested channel. */
...
/**
 * Fork builds never query the official update service. The fork shares
 * `~/.dsh` with stable, so applying an official installer over it would
 * silently replace the fork build (same version line, different appId). All
 * update checks — scheduled and manual — report "no result" instead.
 */
export const FORK_UPDATE_CHECKS_DISABLED = true
...
/**
 * Check the fixed DSH Desktop Fork version endpoint for a release in one channel.
...
export async function checkForDesktopUpdate(
  ...
  // Fork hard gate: never contact the official feed (see FORK_UPDATE_CHECKS_DISABLED).
  if (FORK_UPDATE_CHECKS_DISABLED) return null
