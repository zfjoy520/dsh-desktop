...
function takeDefaultProfile(environment: NodeJS.ProcessEnv): string | undefined {
  ...
  for (const key of Object.keys(environment)) {
    ...
    if (value !== undefined && profileName !== undefined && value !== profileName) {
      throw new Error('dsh-desktop-fork: conflicting default profile environment values')
    }
    profileName ??= value
...
export function desktopCliProfileManifestUrl(
  ...
  if (dirname(profileDirectory) !== profileRoot) {
    throw new Error(`dsh-desktop-fork: unsafe CLI profile path for ${JSON.stringify(profileName)}`)
  }
...
if (isDirectExecution()) {
  void runDesktopDshCli().catch((cause: unknown) => {
    process.stderr.write(`dsh-desktop-fork: failed to start packaged dsh: ${cause instanceof Error ? cause.stack ?? cause.message : String(cause)}\n`)
    process.exitCode = 1
  })
}
