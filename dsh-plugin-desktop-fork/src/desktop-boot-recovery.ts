...
[data-dsh-desktop-fork-recovery] {
...
[data-dsh-desktop-fork-recovery] p {
  margin: 0;
  font-size: 12px;
  line-height: 18px;
}
[data-dsh-desktop-fork-recovery] button {
  min-height: 40px;
  padding: 0 18px;
  border: 0;
  border-radius: 20px;
...
[data-dsh-desktop-fork-recovery] button:hover:not(:disabled),
[data-dsh-desktop-fork-recovery] button:active:not(:disabled) {
  background: var(--dsh-recovery-primary-hover);
}
[data-dsh-desktop-fork-recovery] button:focus-visible {
  outline: 2px solid #5b8def;
  outline-offset: 2px;
}
[data-dsh-desktop-fork-recovery] button:disabled {
  cursor: progress;
  opacity: 0.52;
}
body[data-ds-dark-theme] [data-dsh-desktop-fork-recovery] {
...
@media (max-width: 520px) {
  [data-dsh-desktop-fork-recovery] { width: calc(100vw - 32px); }
  [data-dsh-desktop-fork-recovery] button { width: 100%; }
}
`
...
export const DESKTOP_BOOT_RECOVERY_SCRIPT = `(() => {
  ...
  const description = '部分插件加载失败，可能与当前 DSH 版本不兼容。你可以进入恢复模式卸载有问题的插件，或新建 Profile 后重新启动。 / Some plugins failed to load and may be incompatible with this DSH version. Open Recovery Mode to uninstall the affected plugins or create a new Profile, then restart DSH Desktop Fork.';
  ...
  const attach = () => {
    ...
    if (!root || root.querySelector('[data-dsh-desktop-fork-recovery]')) return;
