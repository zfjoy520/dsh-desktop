...
export class DesktopSetupWizardWindow {
  ...
  async run(): Promise<DesktopSetupWizardResult> {
    if (this.started) throw new Error('dsh-plugin-desktop-fork: Setup Wizard can only run once')
    this.started = true
    if (!isDesktopSetupWizardInput(this.options.input)) {
      throw new TypeError('dsh-plugin-desktop-fork: invalid Setup Wizard input')
    }
