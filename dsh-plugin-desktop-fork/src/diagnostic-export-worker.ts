...
function hostLogEntries(hostLogsDir: string | undefined): LogEntry[] {
  ...
  if (rootStats.isSymbolicLink() || !rootStats.isDirectory()) {
    throw new Error('dsh-plugin-desktop-fork: refusing linked Host log directory')
  }
...
function crashDumpEntries(crashDumpsDir: string | undefined): LogEntry[] {
  ...
  if (rootStats.isSymbolicLink() || !rootStats.isDirectory()) {
    throw new Error('dsh-plugin-desktop-fork: refusing linked crash dump directory')
  }
...
async function createDiagnosticsArchive(data: DiagnosticExportWorkerData): Promise<string> {
  ...
  if (logsStats.isSymbolicLink() || !logsStats.isDirectory()) {
    throw new Error('dsh-plugin-desktop-fork: refusing linked log directory')
  }
  ...
  if (outputStats.isSymbolicLink() || !outputStats.isDirectory()) {
    throw new Error('dsh-plugin-desktop-fork: refusing linked diagnostics directory')
  }
