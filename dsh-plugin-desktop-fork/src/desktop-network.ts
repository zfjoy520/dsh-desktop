...
export function parseDesktopOpenBrowser(value: unknown): boolean {
  ...
  throw new Error('dsh-plugin-desktop-fork: dsh-desktop-fork.openBrowser must be a boolean')
}
...
export function parseDesktopNetworkExposure(value: unknown): DesktopNetworkExposure {
  ...
  throw new Error('dsh-plugin-desktop-fork: dsh-desktop-fork.networkExposure must be "loopback" or "lan"')
}
...
export function desktopLanBrowserUrls(port: number, addresses: readonly string[]): readonly string[] {
  if (!Number.isSafeInteger(port) || port < 1 || port > 65_535) {
    throw new TypeError('dsh-plugin-desktop-fork: LAN HTTPS port must be an integer from 1 through 65535')
  }
  const urls = addresses.map((address) => {
    if (!/^\d{1,3}(?:\.\d{1,3}){3}$/u.test(address)
      || address.split('.').some(part => Number(part) > 255)) {
      throw new TypeError(`dsh-plugin-desktop-fork: invalid LAN IPv4 address ${JSON.stringify(address)}`)
    }
