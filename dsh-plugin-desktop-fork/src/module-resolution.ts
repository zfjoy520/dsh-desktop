...
const PROCESS_RESOLVER = Symbol.for('dsh-plugin-desktop-fork.profile-package-resolver.v1')
...
export function installProfilePackageResolver(profileBaseUrl: string): () => void {
  ...
  if (parsed.protocol !== 'file:' || parsed.search.length > 0 || parsed.hash.length > 0) {
    throw new Error('dsh-plugin-desktop-fork: Profile package resolver requires a plain file URL')
  }
  const profileManifestPath = fileURLToPath(parsed)
  if (basename(profileManifestPath) !== 'package.json') {
    throw new Error('dsh-plugin-desktop-fork: Profile package resolver requires a package.json anchor')
  }
