...
function updateAvailableNotification(locale: DesktopLocale, version: string): DesktopNotification {
  return locale === 'zh'
    ? { title: 'DSH Desktop Fork 有可用更新', body: `版本 ${version} 已可下载。打开 DSH Desktop Fork 即可继续。` }
    : { title: 'DSH Desktop Fork Update Available', body: `Version ${version} is ready to download. Open DSH Desktop Fork to continue.` }
}
