...
export const DESKTOP_LAN_HTTPS_CA_PATH = '/.well-known/dsh-desktop-fork-ca.crt'
...
export class DesktopLanHttpsRuntime {
  ...
  attach(targetPort: number): void {
    if (this.targetPort !== undefined) {
      ...
      throw new Error('dsh-plugin-desktop-fork: LAN HTTPS runtime is already attached to another port')
    }
  ...
  async setEnabled(enabled: boolean): Promise<DesktopLanHttpsRuntimeSnapshot> {
    if (typeof enabled !== 'boolean') {
      throw new TypeError('dsh-plugin-desktop-fork: LAN HTTPS enabled state must be a boolean')
    }
