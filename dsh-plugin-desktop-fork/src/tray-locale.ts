...
const labels: Record<DesktopLocale, Record<DesktopTrayLabelKey, (value: string) => string>> = {
  en: {
    ...
    downloadingUpdate: version => `Downloading DSH Desktop Fork ${version}…`,
    ...
    updateAvailable: version => `DSH Desktop Fork ${version} Available`,
  },
  zh: {
    addProfile: () => '新建 Profile…',
    checkForUpdates: () => '检查更新…',
    checkingForUpdates: () => '正在检查更新…',
    downloadingUpdate: version => `正在下载 DSH Desktop Fork ${version}…`,
    ...
    updateAvailable: version => `DSH Desktop Fork ${version} 可用`,
  },
}
...
export const rendererRecoveryCopy: Record<DesktopLocale, DesktopRestartConfirmationCopy> = {
  en: {
    title: 'Restore DSH Desktop Fork',
    message: 'The interface could not recover automatically.',
    detail: 'Automatic recovery stopped after repeated failures to avoid a restart loop. You can try again without restarting the background service. Unsent input may be lost. Export diagnostics from the tray to investigate. Choose Open DSH Desktop Fork from the tray to return to this prompt later.',
    confirm: 'Try recovery again',
    cancel: 'Not now',
  },
  zh: {
    title: '恢复 DSH Desktop Fork',
    message: '界面未能自动恢复。',
    detail: '自动恢复连续失败，为避免重启循环已暂停。可以再次尝试恢复，无需重启后台服务。未发送的输入可能丢失。请从托盘导出诊断信息以继续调查。稍后可从托盘选择“打开 DSH Desktop Fork”再次打开此提示。',
    confirm: '再次尝试恢复',
    cancel: '暂不处理',
  },
}
...
const restartConfirmationCopy: Record<DesktopLocale, Record<'normal' | 'recovery', DesktopRestartConfirmationCopy>> = {
  en: {
    normal: {
      title: 'Restart DSH Desktop Fork',
      message: 'Restart DSH Desktop Fork now?',
    ...
    recovery: {
      ...
      message: 'Restart DSH Desktop Fork in Recovery Mode?',
  ...
  zh: {
    normal: {
      title: '重启 DSH Desktop Fork',
      message: '现在重启 DSH Desktop Fork？',
    ...
    recovery: {
      title: '重启到恢复模式',
      message: '重启 DSH Desktop Fork 并进入恢复模式？',
