/** Headless-safe npm launcher for the DSH Desktop Fork Electron executable. */
...
export const DESKTOP_CLI_HELP = `Usage: dsh-plugin-desktop-fork [options]

Launch DSH Desktop Fork with the selected Web-capable profile.
...
export function defaultDesktopUserDataDirectory(
  ...
  if (platform === 'win32') {
    ...
    if (appData === undefined || appData.length === 0) {
      throw new Error('APPDATA is unavailable; cannot locate DSH Desktop Fork diagnostics')
    }
...
async function launchElectron(): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(
      ...
      + 'Or use the packaged DSH Desktop Fork application.\n',
    )
    return 1
  }
