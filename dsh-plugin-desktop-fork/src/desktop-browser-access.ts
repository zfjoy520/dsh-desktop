...
export const DESKTOP_RENDERER_ACCESS_HEADER = 'x-dsh-desktop-fork-renderer'

const ACCESS_TOKEN_BYTES = 32
const ACCESS_TOKEN_PATTERN = /^[A-Za-z0-9_-]{43}$/u
const DESKTOP_MARKER_PREFIX = 'dsh-desktop-fork-'
...
export function createDesktopBrowserAccess(
  ...
  if (typeof ordinaryBrowserEnabled !== 'boolean') {
    throw new TypeError('dsh-plugin-desktop-fork: ordinary browser access must be a boolean')
  }
  if (!ACCESS_TOKEN_PATTERN.test(token)) {
    throw new TypeError('dsh-plugin-desktop-fork: renderer access token must be 32 base64url bytes')
  }
